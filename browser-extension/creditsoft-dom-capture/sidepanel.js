...
const API_AUTOMATION_DISCOVERY_PATH =
    '/api/v1/browser-companion/automation-discovery';
const DEFAULT_PROVIDER_KEY = 'smartcredit';
const NEXT_READY_ACCOUNT_PATH = '/api/v1/browser-companion/next-account';
const DISPUTEFOX_LOGIN_URL =
    'https://pulse.disputeprocess.com/jsp/client/login.jsp?cdn/';
...
const PULSE_PROFILE_PROCESS_LANES = PULSE_MIGRATION_LANES.filter(
...
const elements = {
    ...
    recoverySweep: document.getElementById('recoverySweep'),
    ...
    saveDisputeFoxCredentials: document.getElementById(
        'saveDisputeFoxCredentials',
    ),
    ...
    importDisputeFoxInvoices: document.getElementById(
        'importDisputeFoxInvoices',
    ),
    importDisputeFoxAffiliates: document.getElementById(
        'importDisputeFoxAffiliates',
    ),
    importDisputeFoxAutomation: document.getElementById(
        'importDisputeFoxAutomation',
    ),
...
let companionOperationActive = false;
let companionStopRequested = false;

class CompanionStopError extends Error {
    constructor() {
        super('Stopped by user.');
        this.name = 'CompanionStopError';
    }
}
...
function providerCapturePlan(providerKey) {
    switch (providerKey) {
        ...
        case 'identityiq':
            return [
                'identityiq_dashboard',
                'identityiq_report',
                'identityiq_monitoring',
                'identityiq_alerts',
            ];
        default:
            return [];
...
function detectCaptureStep(context) {
    ...
    if (providerKey === 'smartcredit') {
        if (
            url.includes('/member/credit-report/smart-3b/') ||
        ...
        if (
            url.includes('/member/smart-credit-report.htm') ||
        ...
        if (
            url.includes('/member/scores/score-tracker.htm') ||
    ...
    if (providerKey === 'credit_karma') {
        if (
            url.includes('/credit-health/transunion/credit-report') ||
            title.includes('transunion')
        ) {
            return 'credit_karma_transunion';
        }

        if (
            url.includes('/credit-health/equifax/credit-report') ||
            title.includes('equifax')
        ) {
...
function isSmartCreditReactivationContext(context) {
    ...
    return (
        url.includes('/member/account/reactivation.htm') ||
...
async function inspectProviderCredentialProblem(providerKey) {
    ...
    return await executeOnActiveTab(
        (expectedProviderKey) => {
            ...
            const onSmartCredit =
                expectedProviderKey === 'smartcredit' ||
                currentUrl.includes('smartcredit.com') ||
                currentTitle.includes('smartcredit');
            const onSmartCreditAuth =
                onSmartCredit &&
                currentHost === 'auth.smartcredit.com' &&
                currentPath.includes('/oauth2/authorize');
            const loginIdentifier =
                Array.from(document.querySelectorAll('input'))
                    .find((element) => {
                        ...
                        const name =
                            `${element.name || ''} ${element.id || ''} ${element.autocomplete || ''}`.toLowerCase();

                        return (
                            ['email', 'text', 'search'].includes(type) &&
                            (name.includes('loginid') ||
                                name.includes('login') ||
                                name.includes('email') ||
                                name.includes('user'))
                        );
                    ...
                    ?.value?.trim() || '';
            const smartCreditLoginForm = Boolean(
                document.querySelector(
                    'input#loginId[name="loginId"], form[action*="/oauth2/authorize"] input[name="password"]',
                ),
            );
            const alertTexts = Array.from(
                document.querySelectorAll(
                    '[role="alert"], .alert, .text-error, .error, [class*="error"]',
                ),
            )
                .filter(isVisible)
                .map((element) => shortText(element.textContent, 320))
                .filter(Boolean);
            const smartCreditInvalidAlert = alertTexts.find((text) =>
                /invalid\s+login\s+credentials\.?/i.test(text),
            );

            if (
                smartCreditInvalidAlert &&
                onSmartCredit &&
                (onSmartCreditAuth || smartCreditLoginForm)
            ) {
                return {
                    invalidCredentials: true,
                    reason: 'smartcredit_invalid_credentials',
                    message:
                        'SmartCredit rejected the saved login. Update the SmartCredit email/password before running this report pull again.',
                    evidence: smartCreditInvalidAlert,
                    pageSignature:
                        'smartcredit_fusionauth_invalid_login_credentials',
                    login_identifier: loginIdentifier || null,
                };
            }

            const patterns = [
                /invalid\s+login\s+credentials\.?/i,
                /invalid (email|username|login|credentials|password)/i,
                /(email|username|login|password) (is )?(invalid|incorrect)/i,
                /(email|username|login).{0,80}(password).{0,80}(invalid|incorrect|do not match|does not match|don't match|doesn't match)/i,
                /(password).{0,80}(invalid|incorrect|do not match|does not match|don't match|doesn't match)/i,
                /(username|email).{0,80}(invalid|incorrect|not found|not recognized)/i,
                /login failed/i,
                /sign[- ]?in failed/i,
                /unable to (log|sign)[- ]?in/i,
                /could not (log|sign)[- ]?in/i,
                /couldn't (log|sign)[- ]?in/i,
                /we couldn't find (an )?account/i,
                /the information you entered (is )?(invalid|incorrect)/i,
            ];
            ...
            const match = visibleText.match(matched);
            const matchIndex = match?.index ?? haystack.search(matched);
            const start = Math.max(0, matchIndex - 90);
            const end = Math.min(
                visibleText.length,
                matchIndex + (match?.[0]?.length || 0) + 120,
            );
            const excerpt = visibleText.slice(start, end).trim();

            return {
                invalidCredentials: true,
                reason: onSmartCredit
                    ? 'smartcredit_invalid_credentials'
                    : 'provider_invalid_credentials',
                message: onSmartCredit
                    ? 'SmartCredit rejected the saved login. Update the SmartCredit email/password before running this report pull again.'
                    : 'The provider rejected the saved login. Update the provider credentials before running this report pull again.',
                evidence: excerpt.slice(0, 320),
                pageSignature: onSmartCreditAuth
                    ? 'smartcredit_fusionauth_invalid_login_credentials'
                    : 'provider_invalid_credentials_text',
                login_identifier: loginIdentifier || null,
            };
        },
        [providerKey],
...
function nextPendingCaptureStep(account) {
    ...
    return (
        plan.find(
            (stepKey) =>
                !runnerState.completedSteps.includes(
                    completedStepKey(account, stepKey),
                ),
        ) ?? null
    );
}
...
function candidateBaseUrls(value) {
    ...
    const ordered = preferred
        ? [preferred, ...LOCAL_API_CANDIDATES]
        : [...LOCAL_API_CANDIDATES];

    return [
        ...new Set(
            ordered.map((entry) => normalizeBaseUrl(entry)).filter(Boolean),
        ),
    ];
}
...
function companionIsProcessing() {
    return companionOperationActive || runnerState.active || runnerState.busy;
}

function updateProcessingHero() {
    const processing = companionIsProcessing();
    const account = activeRunnerAccount();
    const clientName = account?.client?.display_name
        ? toText(account.client.display_name).trim()
        : '';
    const providerKey = toText(
        account?.provider_account?.provider_key ||
            activePageContext?.provider_key,
    ).trim();
    ...
    if (elements.processingTitle) {
        elements.processingTitle.textContent = processing
            ? runnerState.active && runnerState.queueScope === 'reactivation'
                ? clientName
                    ? `Recovery sweep for ${clientName}`
                    : 'Recovery sweep running'
                : runnerState.active
                  ? clientName
                      ? `Pulling reports for ${clientName}`
                      : `Pulling ${providerName} reports`
                  : disputeFoxImportOpen
                    ? 'Importing DisputeFox data'
                    : 'CreditSoft companion running'
            : 'Provider report pulls';
    }
    ...
    if (elements.processingDetail) {
        elements.processingDetail.textContent = processing
            ? statusText ||
              (runnerState.active && runnerState.queueScope === 'reactivation'
                  ? 'Checking 90+ day recovery candidates. Fired clients and invalid credentials stay excluded.'
                  : runnerState.active
                    ? `Working through ${providerName} and preparing the next report page.`
                    : 'Moving visible rows, profile history, billing, and files into CreditSoft.')
            : 'CreditSoft is waiting for the next SmartCredit or IdentityIQ report pull.';
    }
}
...
function renderActivity() {
    ...
    const items =
        activityEntries.length > 0
            ...
            : [
                  {
                      label: 'Waiting for the next provider report pull.',
                      tone: 'info',
                  },
              ];
...
function detectProviderFromContext(url = '', title = '') {
    ...
    if (
        haystack.includes('creditkarma.com') ||
        haystack.includes('credit karma')
    ) {
        return 'credit_karma';
    }

    if (
        haystack.includes('smartcredit.com') ||
        haystack.includes('smartcredit')
    ) {
        return 'smartcredit';
    }

    if (
        haystack.includes('identityiq.com') ||
        haystack.includes('identityiq')
    ) {
        return 'identityiq';
    }
...
function isUnsupportedBrowserPage(url = '') {
    return /^(chrome|chrome-extension|devtools|about|edge|brave):\/\//i.test(
        toText(url).trim(),
    );
}
...
function isDisputeFoxPulseUrl(url = '') {
    ...
    return (
        value.includes('pulse.disputeprocess.com') ||
...
function isPulseCustomerProfileUrl(url = '') {
    ...
    return (
        isDisputeFoxPulseUrl(value) && value.includes('customer_dashboard.jsp')
    );
}
...
function isPulseCustomerListUrl(url = '') {
    ...
    return (
        isDisputeFoxPulseUrl(value) &&
        value.includes('customer_and_lead_list.jsp')
    );
}
...
function pulseNextMigrationLane(url = '') {
    ...
    const currentIndex = PULSE_MIGRATION_LANES.findIndex(
        (lane) => lane.key === current.key,
    );
...
function disputeFoxPrimaryActionLabel(url = '') {
    if (!isDisputeFoxPulseUrl(url)) {
        return 'Open DisputeFox';
    }

    if (isPulseLoginUrl(url)) {
        return 'Continue DisputeFox login';
    }

    if (isPulseCustomerProfileUrl(url)) {
        return 'Import current profile';
    }

    const lane = pulseMigrationLaneForUrl(url);

    if (lane?.key === 'clients') {
        return 'Import client profiles';
    }
    ...
    if (lane?.label) {
        return `Import ${lane.label}`;
    }

    return 'Import current page';
}
...
function captureLooksLikeClientProfile(capture) {
    ...
    if (isDisputeFoxPulseUrl(url)) {
        return (
            (pageKind === 'profile' || isPulseCustomerProfileUrl(url)) &&
            profileHasClientFields(profile)
        );
    }
...
function captureLooksLikeAutomationDiscovery(capture) {
    ...
    return (
        pageKind.startsWith('automation-') &&
        workflows.some(
            (workflow) =>
                toText(workflow?.name || workflow?.source_identifier).trim() !==
                '',
        )
    );
}
...
function isCaptureReadyPageContext(context) {
    ...
    switch (context?.provider_key) {
        ...
        case 'credit_karma':
            return (
                url.includes('/credit-health/') &&
                url.includes('/credit-report')
            );
        case 'identityiq':
            return (
                url.includes('/dashboard.aspx') ||
                url.includes('/creditreport.aspx') ||
                url.includes('/identity-monitoring') ||
...
function finishCompanionOperation() {
    companionOperationActive = false;
    companionStopRequested = false;
    updateProcessingHero();
    syncFeatureVisibility();
}

function requestCompanionStop() {
    ...
    runnerState.queueScope = 'active';
    companionOperationActive = false;
    nextReadyAccount = null;
    pushActivity(
        'Stop requested. The companion will stop before the next profile or report step.',
        'warn',
    );
    setStatus('Stopping after the current browser step...', 'warn');
    syncForm();
    updateProcessingHero();
}

function throwIfCompanionStopped() {
    if (companionStopRequested) {
        throw new CompanionStopError();
    }
}

function isCompanionStopError(error) {
    return (
        error instanceof CompanionStopError ||
        error?.name === 'CompanionStopError'
    );
}
...
function startRunner(account = null, options = {}) {
    ...
    runnerState.queueScope =
        options.queueScope === 'reactivation' ? 'reactivation' : 'active';
    updateProcessingHero();
}

function activeQueueEmptyMessage() {
    return runnerState.queueScope === 'reactivation'
        ? 'No 90+ day recovery candidates are queued right now. Fired clients and invalid credentials stay excluded.'
        : 'No provider report pull is waiting right now. Reports already pulled today stay skipped until tomorrow.';
}

function activeQueueCompleteMessage() {
    return runnerState.queueScope === 'reactivation'
        ? 'Recovery sweep complete for now.'
        : 'Queue complete for now.';
}
...
async function inspectPageAutomationState() {
    ...
    return await executeOnActiveTab(() => {
        ...
        const loginField = Array.from(fieldRoot.querySelectorAll('input')).find(
            (element) => {
        ...
        const securityQuestionPage =
            url.includes('security-question') ||
            title.includes('security question');
        const securityAnswerField = Array.from(
...
async function submitProviderLogin(account) {
    ...
    const loginValue = toText(
        account?.provider_account?.preferred_login?.value,
    ).trim();
    const passwordValue = toText(
        account?.provider_account?.login_password,
    ).trim();
    const securityAnswerValue = toText(
        account?.provider_account?.security_answer,
    ).trim();
    ...
    return await executeOnActiveTab(
        (credentials) => {
            ...
            const currentTitle = document.title.toLowerCase();
            const securityQuestionPage =
                credentials.providerKey === 'identityiq' &&
                (currentUrl.includes('security-question') ||
                    currentTitle.includes('security question'));
            ...
            if (securityQuestionPage) {
                ...
                if (!answerField || !credentials.securityAnswer) {
                    return {
                        submitted: false,
                        reason: 'missing-security-answer',
                    };
                }
                ...
                const submitButton = Array.from(
                    fieldRoot.querySelectorAll('button, input[type="submit"]'),
                ).find((element) => {
                    if (!isVisible(element)) {
                        return false;
                    }
                    ...
                    const label =
                        `${element.textContent || ''} ${element.getAttribute('value') || ''} ${element.getAttribute('aria-label') || ''}`.toLowerCase();

                    return (
                        /continue|submit|verify|next|answer/.test(label) ||
                ...
                if (submitButton instanceof HTMLElement) {
                    ...
                    return {
                        submitted: true,
                        method: 'security-question-button',
                    };
                }
            ...
            const fieldRoot = passwordField.closest('form') ?? document;
            const loginField = Array.from(
                fieldRoot.querySelectorAll('input'),
            ).find((element) => {
                if (element === passwordField || !isVisible(element)) {
                    return false;
                }
            ...
            const submitButton = Array.from(
                ...
                return (
                    /log ?in|sign ?in|continue|submit/.test(label) ||
                    element.getAttribute('type') === 'submit'
                );
            });
...
async function advanceToCaptureReady(account) {
    ...
    const clientName =
        toText(account?.client?.display_name).trim() || 'the queued client';
    ...
    for (let step = 0; step < 5; step += 1) {
        ...
        if (isCaptureReadyPageContext(activePageContext)) {
            ...
            if (
                currentStep &&
                !runnerState.completedSteps.includes(
                    completedStepKey(account, currentStep),
                )
            ) {
                return true;
            }
            ...
            if (pendingStep) {
                ...
                if (
                    pendingUrl &&
                    toText(activePageContext.url).trim() !== pendingUrl
                ) {
                    pushActivity(
                        `Opening the next ${providerName} page for ${clientName}.`,
                        'info',
                    );
                    setStatus(
                        `Opening the next ${providerName} page for ${clientName}...`,
        ...
        if (!onProviderPage) {
            const pendingStep = nextPendingCaptureStep(account);
            const targetUrl =
                providerStepUrl(providerKey, pendingStep) ||
                providerStartUrlFromAccount(account);

            if (!targetUrl) {
                throw new Error(
                    `No start page is configured for ${providerName} yet.`,
                );
            }

            pushActivity(
                `Opening ${providerName} for ${clientName}.`,
                'success',
            );
        ...
        const credentialProblem =
            await inspectProviderCredentialProblem(providerKey);

        if (credentialProblem?.invalidCredentials) {
            return {
                ready: false,
                credentialProblem,
            };
        ...
        if (
            pageState?.hasSecurityAnswerField &&
            account?.provider_account?.security_answer
        ) {
            ...
            if (submitted?.submitted) {
                pushActivity(
                    `Answering the ${providerName} security question for ${clientName}.`,
                    'success',
                );
                setStatus(
                    `Answering the ${providerName} security question for ${clientName}...`,
        ...
        if (
            ...
            if (submitted?.submitted) {
                pushActivity(
                    `Signing into ${providerName} for ${clientName}.`,
                    'success',
                );
                setStatus(
                    `Signing into ${providerName} for ${clientName}.`,
        ...
        if (pageState?.hasLoginTrigger) {
            ...
            if (clicked?.clicked) {
                pushActivity(
                    `Opening the ${providerName} sign-in step.`,
                    'info',
                );
                setStatus(
                    `Opening the ${providerName} sign-in step...`,
        ...
        if (targetUrl && toText(activePageContext.url).trim() !== targetUrl) {
            pushActivity(
                `Redirecting to the ${providerName} report page.`,
                'info',
            );
            setStatus(
                `Redirecting to the ${providerName} report page...`,
...
function syncPairingState() {
    ...
    if (elements.pairedDot) {
        ...
        elements.pairedDot.title = paired
            ? connectionLabel()
            : 'No CreditSoft API key saved yet';
...
function renderNextClient() {
    if (
        !elements.nextClientCard ||
        !elements.nextClientEmpty ||
        !elements.goCapture
    ) {
        return;
    }
    ...
    const pairedAndReady =
        Boolean(settingsCache.office_token) && connectionState === 'ready';

    if (isImportMenuContext()) {
        nextReadyAccount = null;

        if (elements.nextSectionEyebrow) {
            elements.nextSectionEyebrow.textContent = 'Sources';
        }

        if (elements.nextSectionTitle) {
            elements.nextSectionTitle.textContent = 'Reports vs legacy imports';
        }
        ...
        elements.nextClientEmpty.textContent =
            'Use Pull next report for SmartCredit and IdentityIQ. Use Legacy imports for old CRM data.';
        elements.goCapture.textContent = 'Choose source above';
    ...
    if (isDisputeFoxImportContext()) {
        ...
        if (elements.nextSectionEyebrow) {
            elements.nextSectionEyebrow.textContent = 'Legacy import';
        }

        if (elements.nextSectionTitle) {
            elements.nextSectionTitle.textContent = 'DisputeFox';
        }

        if (elements.nextSectionHelp) {
            elements.nextSectionHelp.hidden = false;
            elements.nextSectionHelp.textContent =
                'DisputeFox imports old CRM data. SmartCredit and IdentityIQ report pulls stay in the report queue.';
        }

        elements.nextClientCard.hidden = true;
        elements.nextClientCard.innerHTML = '';
        elements.nextClientEmpty.hidden = false;
        elements.nextClientEmpty.textContent = isDisputeFoxPulseUrl(
        ...
        elements.goCapture.textContent = disputeFoxPrimaryActionLabel(
            activePageContext.url,
        );
    ...
    if (elements.nextSectionEyebrow) {
        elements.nextSectionEyebrow.textContent = 'Report pulls';
    }

    if (elements.nextSectionTitle) {
        elements.nextSectionTitle.textContent =
            'SmartCredit / IdentityIQ queue';
    }
    ...
    elements.goCapture.textContent = captureReady
        ? 'Capture this report'
        : 'Pull next report';
    ...
    if (!onProviderPage) {
        ...
        elements.nextClientCard.innerHTML = '';
        elements.nextClientEmpty.hidden = false;
        elements.nextClientEmpty.textContent = isUnsupportedBrowserPage(
            activePageContext.url,
        )
            ? 'Pull next report opens the next provider report page. Browser internal pages cannot be captured.'
            : 'Pull next report opens the next queued SmartCredit or IdentityIQ report page.';
    ...
    if (!client || !provider) {
        ...
        elements.nextClientEmpty.textContent = settingsCache.office_token
            ? `No ${providerLabel(activePageContext.provider_key)} report pull is queued.`
            : 'Save a CreditSoft API key, then the next report pull will appear here.';
        elements.goCapture.disabled = true;
        return;
    }
    ...
    elements.nextClientCard.innerHTML = `
        ...
        <div class="selected-actions">
          <div class="meta">${latestCycle || 'Loaded from the provider report queue.'}</div>
    ...
    if (!captureReady && elements.nextClientEmpty) {
        elements.nextClientEmpty.hidden = false;
        elements.nextClientEmpty.textContent = `Pull next report will open ${providerLabel(provider.provider_key)} for ${client.display_name}.`;
...
function syncIntegrationMenu() {
    ...
    if (elements.integrationMenuToggle) {
        elements.integrationMenuToggle.setAttribute(
            'aria-expanded',
            integrationMenuOpen ? 'true' : 'false',
        );
    }
}
...
function syncCredentialForm() {
    if (elements.disputefoxUsername) {
        elements.disputefoxUsername.value =
            settingsCache.disputefox_username || '';
    }

    if (elements.disputefoxPassword) {
        elements.disputefoxPassword.value =
            settingsCache.disputefox_password || '';
    }
}

function syncFeatureVisibility() {
    const enabled = Boolean(
        featureFlags.client_sync && featureFlags.disputefox_credentials,
    );
    ...
    if (elements.stopRun) {
        elements.stopRun.disabled = !(
            companionOperationActive ||
            runnerState.active ||
            runnerState.busy
        );
    }

    if (elements.recoverySweep) {
        elements.recoverySweep.disabled =
            !ready || runnerState.active || runnerState.busy;
    }
...
function startContextWatcher() {
    ...
    contextWatcherId = window.setInterval(() => {
        void (async () => {
            const shouldLoadProviderQueue =
                !runnerState.active &&
                !integrationMenuOpen &&
                !disputeFoxImportOpen;

            await syncActiveContext({
                loadQueue: shouldLoadProviderQueue,
                quiet: true,
            });
...
async function captureActiveTab() {
    ...
    if (!tab?.id) {
        throw new Error(
            'No active tab found. Open the page you want to capture.',
        );
    }
    ...
    const results = await chrome.scripting.executeScript({
        ...
        func: async () => {
            ...
            const isPulsePage = /pulse\.disputeprocess\.com$/i.test(
                window.location.hostname,
            );
            const hasProfileMarker =
                /customer_dashboard\.jsp/i.test(currentUrl) ||
                Boolean(
                    document.querySelector(
                        'title[customer_name], #client_card_info_name, .client_card_info_name',
                    ),
                );
            const ignoredPulseTable = (table) =>
                /^(DFMessage|DFAllMessage|DFPortalMessage|DFLeadChat|DFClientsDocuments|bureauForFreezeLetter)$/i.test(
                    table.id || '',
                );
            const hasUsableRecordRows = (table) => {
                if (
                    !(table instanceof HTMLTableElement) ||
                    ignoredPulseTable(table)
                ) {
                    return false;
                }
            ...
            const hasRecordTable =
                !hasProfileMarker &&
                Array.from(
                    document.querySelectorAll(
                        '#clientsORleadsDatatable, #affiliate_table, #client_invoice_table, table.dataTable, table.datatable, table',
                    ),
                ).some(hasUsableRecordRows);
            const pageKind = hasProfileMarker
                ? 'profile'
                : hasRecordTable
                  ? 'record-list'
                  : 'page';
            ...
            const previewPathFromOnclick = (value) => {
                const match = String(value || '').match(
                    /(?:previewDocument|editUploadedDocument)\('([^']+)'/i,
                );
            ...
            const normalizePulseDocument = (doc, source = 'pulse-api') => {
                const sourcePath = String(
                    doc?.client_document_url || doc?.source_path || '',
                ).trim();
                const downloadUrl = absoluteUrl(
                    doc?.client_document_full_url || doc?.download_url || '',
                );
                const sourcePreviewPath =
                    sourcePath && !/^\/?document\?/i.test(sourcePath)
                        ? `/static-resources/client_documents/${sourcePath.replace(/^\/+/, '')}`
                        : '';
                const previewUrl = absoluteUrl(
                    doc?.preview_url || sourcePreviewPath,
                );
                const fileName = String(
                    doc?.file_name ||
                        filenameFromDocumentUrl(downloadUrl) ||
                        filenameFromPath(sourcePath) ||
                        '',
                ).trim();
                const title = short(
                    doc?.client_document_name_text ||
                        doc?.title ||
                        titleFromDocumentUrl(downloadUrl) ||
                        fileName ||
                        'DisputeFox document',
                    255,
                );
                ...
                return {
                    ...
                    source,
                    source_document_uid: short(
                        doc?.client_document_u_id ||
                            doc?.source_document_uid ||
                            uidFromDocumentUrl(downloadUrl) ||
                            '',
                        255,
                    ),
                    source_client_uid: short(
                        doc?.client_uid ||
                            doc?.source_client_uid ||
                            clientUidFromPage(),
                        255,
                    ),
                    title,
                    ...
                    mime_type: short(
                        doc?.mime_type ||
                            mimeFromFilename(fileName || sourcePath),
                        255,
                    ),
                    file_size: Number(doc?.file_size || 0) || 0,
                    uploaded_at_label: short(
                        doc?.client_document_date ||
            ...
            const detectPulseDocuments = async () => {
                ...
                for (const row of Array.from(
                    ...
                    if (
                        !downloadUrl ||
                        !['clientDocument', 'downloadCreditAuditReport'].includes(
                            method,
                        )
                    ) {
                        continue;
                    ...
                    const downloadClientUid =
                        downloadUrl.searchParams.get('clientUid') ||
                        downloadUrl.searchParams.get('client_u_id') ||
                        '';

                    if (
                        downloadClientUid &&
                        clientUid &&
                        downloadClientUid !== clientUid
                    ) {
                        continue;
                    }

                    const rowTitle =
                        row.querySelector('.documents-row-name-text')
                            ?.textContent || titleFromDocumentUrl(downloadHref);
                    const previewPath = Array.from(
                        row.querySelectorAll('[onclick]'),
                    )
                        .map((element) =>
                            previewPathFromOnclick(
                                element.getAttribute('onclick'),
                            ),
                        )
                        .find(Boolean);
                    const isAudit =
                        method === 'downloadCreditAuditReport' ||
                        /audit report/i.test(rowTitle);
                    const isCreditReport =
                        downloadUrl.searchParams.get('isCreditReport') === '1' ||
                        (row.closest('#progressCreditReportUploadedDiv') &&
                            !isAudit);

                    addDocument(
                        normalizePulseDocument(
                            {
                                source_document_uid:
                                    row
                                        .querySelector('[data-docid]')
                                        ?.getAttribute('data-docid') ||
                                    uidFromDocumentUrl(downloadHref),
                                source_client_uid:
                                    downloadClientUid || clientUid,
                                client_document_full_url: downloadHref,
                                preview_url: previewPath
                                    ? `/static-resources/client_documents/${previewPath.replace(/^\/+/, '')}`
                                    : '',
                                client_document_name_text: rowTitle,
                                client_document_date:
                                    row
                                        .querySelector('.documents-row-date i')
                                        ?.getAttribute('title') ||
                                    row.querySelector('.documents-row-date')
                                        ?.textContent ||
                                    '',
                                category: isAudit
                                    ? 'audit_report'
                ...
                for (const row of Array.from(
                    document.querySelectorAll(
                        '.messages-modal-document-list .df-sidebar-document-row',
                    ),
                ).slice(0, 80)) {
                    const args = Array.from(
                        String(row.getAttribute('onclick') || '').matchAll(
                            /'([^']*)'/g,
                        ),
                    ).map((match) => match[1]);
                    ...
                    addDocument(
                        normalizePulseDocument(
                            {
                                source_document_uid:
                                    (row.id || '').replace(/^document_/, '') ||
                                    args[2] ||
                                    '',
                                source_client_uid: args[3] || '',
                                client_document_url: args[0] || '',
                                client_document_full_url: args[1] || '',
                                client_document_name_text:
                                    row.querySelector(
                                        '.messages-modal-client-doc',
                                    )?.textContent || '',
                                client_document_date:
                                    row.querySelector(
                                        '.messages-modal-item-time',
                                    )?.textContent || '',
                            },
                            'pulse-sidebar',
                        ),
                ...
                const selectedDownload =
                    document
                        .querySelector('#df_downloadDocument')
                        ?.getAttribute('href') || '';
                const selectedPreview =
                    document
                        .querySelector(
                            '#documentPreviewIdSidebar, #documentPreviewImgIdSidebar',
                        )
                        ?.getAttribute('src') || '';
                        ...
                        normalizePulseDocument(
                            {
                                source_document_uid:
                                    document
                                        .querySelector('#df_markDocumentViewed')
                                        ?.getAttribute(
                                            'data_client_document_u_id',
                                        ) || '',
                                source_client_uid:
                                    document
                                        .querySelector('#df_markDocumentViewed')
                                        ?.getAttribute('data-clientuid') || '',
                                client_document_full_url: selectedDownload,
                                preview_url: selectedPreview,
                                client_document_name_text:
                                    document.querySelector(
                                        '.messages-modal-document-list .activeDoconSidebar .messages-modal-client-doc',
                                    )?.textContent ||
                                    filenameFromPath(
                                        selectedDownload || selectedPreview,
                                    ),
                                client_document_date:
                                    document.querySelector(
                                        '.messages-modal-document-list .activeDoconSidebar .messages-modal-item-time',
                                    )?.textContent || '',
                            },
                ...
                if (
                    /pulse\.disputeprocess\.com$/i.test(
                        window.location.hostname,
                    ) &&
                    clientUid
                ) {
                    try {
                        const response = await fetch(
                            `${window.location.origin}/clientDashboard`,
                            {
                                method: 'POST',
                                credentials: 'include',
                                headers: {
                        ...
                        for (const doc of Array.isArray(payload?.data)
                            ? payload.data
                            : []) {
                            addDocument(
                                normalizePulseDocument(doc, 'pulse-api'),
                            );
                        }
                    } catch (_) {
            ...
            const cleanProfileTitle = (value) => {
                const cleaned = short(value, 160)
                    .replace(/\s*\|\s*.*$/, '')
                    .replace(
                        /\s*[-–]\s*(account|overview|profile|details|client)\s*$/i,
                        '',
                    )
                    .trim();
            ...
            const aliases = {
                ...
                phone: [
                    ...
                    'cell',
                    'telephone',
                    'cell phone only for sms security codes',
                ],
                address_line_1: [
                    'address',
                    'current address',
                ...
                source_record_int_id: [
                    'source record integer id',
                    'external integer id',
                    'client int id',
                    'customer int id',
                    'pulse client int id',
            ...
            const records = Array.from(
                ...
                .map((el) => {
                    ...
                    if (
                        type === 'password' &&
                        !(isPulsePage && pageKind === 'profile')
                    )
                        return null;
                    if (
                        !visibleEnough(el) &&
                        !pulseProfileHiddenFieldAllowed(el)
                    )
                        return null;
            ...
            if (pageKind === 'profile') {
                const titleCustomerName = document
                    .querySelector('title[customer_name]')
                    ?.getAttribute('customer_name');
                const cardName = document.querySelector(
                    '#client_card_info_name, .client_card_info_name',
                )?.textContent;
                const titleName = cleanProfileTitle(document.title);

                addTextRecord(
                    'customer name',
                    titleCustomerName || cardName || titleName,
                    'profile-marker',
                );
                ...
                const forcePaymentRaw =
                    window.localStorage?.getItem('forcePaymentData');
                ...
                for (const row of Array.from(
                    document.querySelectorAll(
                        '.customer-side-panel-ssn, .profile-row, .detail-row, .info-row',
                    ),
                ).slice(0, 120)) {
                    const label = row.querySelector(
                        '.customer-side-panel-ssn-left, .label, .field-label, dt',
                    )?.textContent;
                    const value = row.querySelector(
                        '.customer-side-panel-ssn-right, .value, .field-value, dd',
                    )?.textContent;
                ...
                const selectorPairs = [
                    ...
                    [
                        '#customer-side-panel-ssn-right-home-phone-number',
                        'home phone',
                    ],
                    ['#customer-side-panel-ssn-right-email', 'email'],
                    [
                        '#customer-side-panel-client-id-right-started',
                        'source record integer id',
                    ],
                ];
            ...
            const textFrom = (selector, root = document) =>
                short(
                    root.querySelector(selector)?.innerText ||
            ...
            const detectPulseInvoiceDetail = () => {
                ...
                const modalText = short(
                    modal.innerText || modal.textContent || '',
                    2000,
                );
                const invoiceId =
                    textFrom('#invoice_id_number', modal) ||
                    textFrom('#invoiceIdDisplay', modal) ||
                    modalText.match(
                        /Invoice\s*(?:ID\s*Number|#)\s*([A-Z0-9-]+)/i,
                    )?.[1] ||
                    '';
                const clientName =
                    textFrom('#invoice_customer_name', modal) ||
                    textFrom('#clientName', modal);
                const clientUid = String(
                    modal.querySelector('#client_id_number')?.dataset
                        ?.client_id || '',
                ).trim();
                const sourceInvoiceId = String(
                    modal.querySelector('#invoice_id_number')?.dataset
                        ?.invoice_id || '',
                ).trim();
                const dueDate =
                    textFrom('#total_due_date', modal) ||
                    textFrom('#dueDateDisplay', modal) ||
                    textFrom('#invoice_created_date', modal);
                const total =
                    textFrom('#grand_total', modal) ||
                ...
                const lineItems = Array.from(
                    modal.querySelectorAll(
                        '.view-invoice-min-height .view-invoice-item-row, #tbody tr',
                    ),
                )
                    .map((row) => {
                        const cells = Array.from(row.querySelectorAll('th, td'))
                            .map((cell) =>
                                short(cell.innerText || cell.textContent, 250),
                            )
                            .filter(Boolean);
                        ...
                        if (cells.length > 0) {
                            return {
                                ...
                                discount: cells[4] || '',
                                total:
                                    cells[5] || cells[cells.length - 1] || '',
                            };
                        }

                        const description = short(
                            row.querySelector('.view-invoice-tem')?.innerText ||
                                row.querySelector('.view-invoice-tem')
                                    ?.textContent ||
                                '',
                            250,
                        );
                        const amount = short(
                            row.querySelector('.text-block-81')?.innerText ||
                                row.querySelector('.text-block-81')
                                    ?.textContent ||
                                '',
                            120,
                        );
                ...
                return {
                    ...
                    source_record_id:
                        sourceInvoiceId || invoiceId.replace(/^DISF/i, ''),
                    source_record_int_id: sourceInvoiceId,
                    profile_url: clientUid
                        ? absoluteUrl(`customer_dashboard.jsp?id=${clientUid}`)
                        : '',
                    values: {
                        ...
                        Amount: total,
                        Status:
                            textFrom('.view-invoice-no-due', modal) ||
                            'invoice_detail',
                        'Sub Total':
                            textFrom('#sub-total', modal) ||
                            textFrom('#subTotalDisplay', modal),
                        'Sales Tax': textFrom('#salesTaxDisplay', modal),
                        Notes: textFrom('#notesForCustomerText', modal),
                        Company:
                            textFrom('#invoice_company_name', modal) ||
                            textFrom('#companyName', modal),
                        'Client Address': [
                            textFrom('#invoice_customer_street', modal) ||
                                textFrom('#clientAddress', modal),
                            textFrom('#invoice_customer_city', modal) ||
                                textFrom('#clientCityAndState', modal),
            ...
            for (const table of dataTables) {
                const headers = Array.from(table.querySelectorAll('thead th'))
                    .map((cell) =>
                        short(cell.innerText || cell.textContent, 100),
                    )
                ...
                for (const row of Array.from(
                    ...
                    const cells = Array.from(row.querySelectorAll('td')).map(
                        (cell) =>
                            short(cell.innerText || cell.textContent, 500),
                    );
                    ...
                    const record = {
                        ...
                        source_record_id:
                            row
                                .querySelector('[data-clientid]')
                                ?.getAttribute('data-clientid') ||
                            row
                                .querySelector('input.client-list-check[value]')
                                ?.getAttribute('value') ||
                            '',
                        source_record_int_id:
                            row
                                .querySelector('[data-clientintid]')
                                ?.getAttribute('data-clientintid') || '',
                        profile_url:
                            row.querySelector(
                                'a[href*="customer_dashboard.jsp?id="]',
                            )?.href || '',
                        values: {},
                    };

                    cells.forEach((value, index) => {
                        record.values[headers[index] || `Column ${index + 1}`] =
                            value;
                    });
            ...
            for (const row of Array.from(document.querySelectorAll('tr')).slice(
                0,
                600,
            )) {
                const cells = Array.from(row.querySelectorAll('th, td'))
            ...
            for (const node of Array.from(
                document.querySelectorAll('p, li, div, span'),
            ).slice(0, 1600)) {
            ...
            const keyForLabel = (value) =>
                String(value ?? '')
                    ...
                    .replace(/[^a-z0-9]+/g, '_')
                    .replace(/^_+|_+$/g, '');
            const cleanAutomationLabel = (value, limit = 160) =>
                short(
                    String(value ?? '')
            ...
            const selectedText = (selector) => {
                ...
                return cleanAutomationLabel(
                    option?.textContent || option?.value || '',
                );
            };
            const automationCatalogFromOptions = (selector, source) =>
                uniqueBy(
                    Array.from(document.querySelectorAll(`${selector} option`))
                        ...
                        .filter(
                            (item) =>
                                item.key && item.label && item.key !== 'select',
                        ),
                    (item) => item.key,
                ).slice(0, 140);
            const automationActionCatalog = () => {
                ...
                const items = Array.from(
                    document.querySelectorAll(
                        '.dropdown-item, .df-steps-item-title, .condition-name',
                    ),
                )
                    .map((node) =>
                        cleanAutomationLabel(
                ...
                return uniqueBy(
                    [
                        ...known.filter((label) => items.includes(label)),
                        ...items,
                    ].map((label) => ({
            ...
            const automationStepActions = (container) =>
                uniqueBy(
                    Array.from(
                        container.querySelectorAll(
                            '.df-steps-item-title, .condition-name, .dropdown-item',
                        ),
                    )
                        .map((node) =>
                            cleanAutomationLabel(
                        ...
                        .map((label) =>
                            label.replace(/\s+Message$/i, ' Message'),
                        ),
                    (label) => label,
                ).slice(0, 20);
            const automationSteps = () => {
                const detailSteps = Array.from(
                    document.querySelectorAll('.workflow-step-section'),
                )
                    .slice(0, 100)
                    .map((section) => {
                        const title = cleanAutomationLabel(
                            section.querySelector('.af-steps-title')
                                ?.textContent,
                            160,
                        );
                        const timing = cleanAutomationLabel(
                            section.querySelector('.af-steps-starts-text')
                                ?.textContent,
                            120,
                        );
                        const status = cleanAutomationLabel(
                            section.querySelector('.af-steps-status')
                                ?.textContent,
                            80,
                        );
                ...
                return Array.from(
                    document.querySelectorAll('.autofox-steps-div'),
                )
                    .slice(0, 100)
                    .map((section) => ({
                        title: cleanAutomationLabel(
                            section
                                .closest('tr')
                                ?.querySelector('.client-grid-name')
                                ?.textContent || 'Automation workflow',
                            160,
                        ),
                        timing: '',
                        status: cleanAutomationLabel(
                            section.querySelector(
                                '.type_name, .fox-status-active, .fox-status-inactive',
                            )?.textContent,
                            80,
                        ),
                        actions: automationStepActions(section),
            ...
            const detectAutomationDiscovery = () => {
                ...
                const legacyAutomationSlug = ['auto', 'fox'].join('');
                const legacyAutomationTypeLabel = ['Auto', 'Fox Type'].join('');
                const hasAutomationMarker =
                    lowerUrl.includes(legacyAutomationSlug) ||
                    lowerTitle.includes(legacyAutomationSlug) ||
                    pageText
                        .toLowerCase()
                        .includes(`${legacyAutomationSlug} workflow`) ||
                    pageText.toLowerCase().includes('automation workflow') ||
                    Boolean(
                        document.querySelector(
                            '#workflow_name, #workflow_service_Table, #workflowActiveStatus',
                        ),
                    );
                ...
                try {
                    autofoxId =
                        new URL(currentUrl).searchParams.get('autofox_id') ||
                        '';
                } catch (_) {
                    autofoxId = '';
                }
                ...
                const conditionCatalog = automationCatalogFromOptions(
                    'select.conditions',
                    'start-condition',
                );
                const suspensionCatalog = automationCatalogFromOptions(
                    'select.conditionsSuspends',
                    'suspension-condition',
                );
                const pageKind = document.querySelector(
                    '#workflow_name, #workflowActiveStatus, .workflow-step-section',
                )
                    ? 'automation-workflow'
                    : 'automation-workflow-list';
                const labelSample = bodyLines
                    .filter((line) =>
                        /autofox|workflow|condition|action|step|invoice|client|lead/i.test(
                            line,
                        ),
                    )
                    .slice(0, 40);
                const listWorkflows = Array.from(
                    document.querySelectorAll(
                        '#workflow_service_Table tbody tr',
                    ),
                )
                    .slice(0, 120)
                    .map((row) => {
                        ...
                        const editLink = row.querySelector(
                            'a[href*="createautofox.jsp"]',
                        );
                        const name = cleanAutomationLabel(
                            info.querySelector('.client-grid-name')
                                ?.textContent,
                            220,
                        );
                        const sourceIdentifier =
                            workflowIdFromHref(editLink) ||
                            row
                        ...
                        return name
                            ? {
                                  ...
                                  created_label: workflowFieldFromText(
                                      text,
                                      'Created',
                                  ),
                                  workflow_type:
                                      workflowFieldFromText(
                                          text,
                                          'Automation Type',
                                      ) ||
                                      workflowFieldFromText(
                                          text,
                                          legacyAutomationTypeLabel,
                                      ),
                                  start_condition: workflowFieldFromText(
                                      text,
                                      'Start Condition',
                                  ),
                                  category: '',
                    ...
                    .filter(Boolean);
                const workflowNameInput =
                    document.querySelector('#workflow_name');
                const detailName = cleanAutomationLabel(
                    workflowNameInput?.value ||
                        workflowNameInput?.getAttribute('placeholder'),
                    255,
                );
                const detailWorkflow =
                    detailName ||
                    autofoxId ||
                    document.querySelector('#workflowActiveStatus')
                        ? {
                              source_identifier:
                                  autofoxId ||
                                  document.querySelector(
                                      '#workflow_id_main, #workflow_id_step, input[name="workflow_id"]',
                                  )?.value ||
                                  '',
                              name:
                                  detailName ||
                              ...
                              workflow_type: cleanAutomationLabel(
                                  document
                                      .querySelector(
                                          'input[name="workflowFor"]:checked',
                                      )
                                      ?.closest('label')?.textContent,
                                  120,
                              ),
                              start_condition: cleanAutomationLabel(
                                  document
                                      .querySelector(
                                          'input[name="workflowStart"]:checked',
                                      )
                                      ?.closest('label')?.textContent,
                                  120,
                              ),
                ...
                return {
                    source_system: 'disputefox',
                    source_product: 'Automation',
                    page_kind: pageKind,
                    workflow: detailWorkflow,
                    workflows,
                    condition_catalog: uniqueBy(
                        [...conditionCatalog, ...suspensionCatalog],
                        (item) => item.key,
                    ),
                    action_catalog: actionCatalog,
                    label_sample: labelSample,
            ...
            const documents =
                pageKind === 'profile' ? await detectPulseDocuments() : [];
            const fields = {};
            const rawFields = [];
            ...
            for (const record of records) {
                ...
                for (const [target, targetAliases] of Object.entries(aliases)) {
                    if (
                        pageKind === 'record-list' &&
                        !['source_record_id', 'source_record_int_id'].includes(
                            target,
                        )
                    ) {
                        continue;
                    ...
                    if (
                        ...
                        targetAliases.some(
                            (alias) =>
                                haystack === alias || haystack.includes(alias),
                        )
                    ) {
            ...
            if (!fields.full_name && pageKind === 'profile') {
                const titleName = cleanProfileTitle(
                    document
                        .querySelector('title[customer_name]')
                        ?.getAttribute('customer_name') || document.title,
                );
            ...
            if (pageKind === 'profile') {
                ...
                const profileOverrides = {
                    ...
                    date_of_birth: profileText(
                        '#customer-side-panel-ssn-right-dob',
                    ),
                    email: profileText('#customer-side-panel-ssn-right-email'),
                    phone:
                        profileText(
                            '#customer-side-panel-ssn-right-cellnumber',
                        ) ||
                        profileText(
                            '#customer-side-panel-ssn-right-home-phone-number',
                        ),
                    address_line_1: profileText(
                        '.client_card_info_address',
                    ).replace(/\s*,?\s*Kansas City/i, ', Kansas City'),
                    source_record_id: sourceRecordId,
                    source_record_int_id: profileText(
                        '#customer-side-panel-client-id-right-started',
                    ),
                    assigned_to: profileText(
                        '#customer-side-panel-assignedto-agent-name',
                    ),
                };
    ...
    for (const capture of captures) {
        for (const [key, value] of Object.entries(
            capture.structured_customer?.fields || {},
        )) {
    ...
    const pageKind = pageKinds.includes('profile')
        ? 'profile'
        : pageKinds.includes('record-list')
          ? 'record-list'
          : 'page';
    const structuredAutomation =
        automationCaptures.length > 0
            ? {
                  ...automationCaptures[0],
                  workflows: automationCaptures
                      .flatMap((capture) => capture.workflows || [])
                      .slice(0, 160),
                  condition_catalog: automationCaptures
                      .flatMap((capture) => capture.condition_catalog || [])
                      .slice(0, 200),
                  action_catalog: automationCaptures
                      .flatMap((capture) => capture.action_catalog || [])
                      .slice(0, 120),
              }
            : null;
...
async function fetchNextClient(settings, pageContext, options = {}) {
    ...
    const excludeProviderAccountId = Number.isFinite(
        options?.excludeProviderAccountId,
    )
    ...
    if (excludeProviderAccountId) {
        params.set(
            'exclude_provider_account_id',
            String(excludeProviderAccountId),
        );
    }
    ...
    if (queueScope !== 'active') {
        params.set('queue_scope', queueScope);
    }
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'Could not load clients from CreditSoft.',
            ),
        );
    }
...
async function fetchQueuedAccountForContext(
    settings,
    {
        preferCurrentPage = true,
        excludeProviderAccountId = null,
        forceUpdate = false,
        queueScope = 'active',
    } = {},
) {
    const requestContext =
        preferCurrentPage && isSupportedProviderPageContext(activePageContext)
            ? activePageContext
            : {
                  provider_key: null,
                  page_url: '',
...
function extractNextReadyClient(parsed) {
    const payload =
        parsed?.data ?? parsed?.client ?? parsed?.next_client ?? parsed;
...
async function loadNextClient({ quiet = false } = {}) {
    ...
    if (!onProviderPage) {
        ...
        const helperMessage = isUnsupportedBrowserPage(activePageContext.url)
            ? 'Browser internal pages cannot be captured. Pull next report opens the next provider report page.'
            : 'Ready.';
    ...
    if (!quiet) {
        pushActivity(
            `Checking CreditSoft for the next ${laneLabel} report pull.`,
        );
        setStatus(`Loading next ${laneLabel} report pull...`);
    }
    ...
    try {
        ...
        if (!nextReadyAccount?.client) {
            setStatus(
                `No ${laneLabel}-ready client is queued right now. Anything already imported today stays skipped until tomorrow.`,
                'info',
            );
        ...
        if (!quiet) {
            pushActivity(
                `Loaded ${nextReadyAccount.client.display_name} for a ${laneLabel} report pull.`,
                'success',
            );
        }
        ...
        setStatus(
            isCaptureReadyPageContext(activePageContext)
                ? `Ready to capture this ${laneLabel} report for ${nextReadyAccount.client.display_name}.`
                : `Pull next report will open ${laneLabel} for ${nextReadyAccount.client.display_name}.`,
        ...
        connectionState =
            ...
            error.message.includes('valid CreditSoft API key')
                ? 'error'
                : connectionState;
        syncForm();
        ...
        if (!quiet) {
            pushActivity(
                error instanceof Error
                    ? error.message
                    : 'Could not load the next report pull.',
                'error',
            );
            setStatus(
                error instanceof Error
                    ? error.message
                    : 'Could not load report pulls.',
                'error',
            );
        }
    }
...
async function markSmartCreditNeedsPaymentAndAdvance(settings, account) {
    ...
    if (!Number.isFinite(providerAccountId)) {
        throw new Error(
            'CreditSoft could not identify the SmartCredit provider account to mark.',
        );
    }

    pushActivity(
        `${clientName} landed on SmartCredit reactivation. Marking the account and skipping it.`,
        'warn',
    );
    setStatus(
        `${clientName} needs SmartCredit payment/reactivation. Skipping to the next client...`,
        'warn',
    );
    ...
    await postProviderAccountStatus(
        {
            ...
            message:
                'SmartCredit redirected the companion to account reactivation, so this client likely needs to renew or pay SmartCredit before reports can be pulled.',
    ...
    const nextAccount = await fetchQueuedAccountForContext(settings, {
        ...
        queueScope: runnerState.queueScope,
    });
    const logoutUrl = providerLogoutUrlFromAccount(account);

    if (!nextAccount?.client || !nextAccount?.provider_account) {
        const completeMessage = activeQueueCompleteMessage();
        stopRunner();
        setRunnerAccount(null);
        pushActivity(
            `${clientName} was noted as needing SmartCredit payment/reactivation. ${completeMessage}`,
            'success',
        );
        setStatus(
            `${clientName} needs SmartCredit payment/reactivation. ${completeMessage}`,
            'success',
        );
    ...
    pushActivity(
        `Continuing with ${clientNameFromAccount(nextAccount)}.`,
        'info',
    );
    setStatus(
        `Continuing with ${clientNameFromAccount(nextAccount)}...`,
    ...
    const targetUrl =
        providerStepUrl(
            toText(nextAccount?.provider_account?.provider_key),
            pendingStep,
...
async function markProviderInvalidCredentialsAndAdvance(
    settings,
    account,
    credentialProblem = {},
) {
    ...
    if (!Number.isFinite(providerAccountId)) {
        throw new Error(
            `CreditSoft could not identify the ${providerName} provider account to mark.`,
        );
    }

    pushActivity(
        `${providerName} rejected the saved login for ${clientName}. Marking credentials and skipping it.`,
        'warn',
    );
    setStatus(
        `${clientName} needs updated ${providerName} credentials. Skipping to the next client...`,
        'warn',
    );
    ...
    await postProviderAccountStatus(
        {
            ...
            reason:
                ...
                (providerKey === 'smartcredit'
                    ...
                    : 'provider_invalid_credentials'),
            message:
                credentialProblem.message ||
                `${providerName} rejected the saved login. Update the provider credentials before reports can be pulled.`,
            page_url: activePageContext.url,
            page_title: activePageContext.title,
            worker_id: settings.worker_id,
            companion: {
                name: 'CreditSoft Browser Companion',
                version: chrome.runtime.getManifest().version,
                evidence: credentialProblem.evidence || null,
                page_signature: credentialProblem.pageSignature || null,
                login_identifier: credentialProblem.login_identifier || null,
            },
        },
        settings,
    );
    ...
    if (!nextAccount?.client || !nextAccount?.provider_account) {
        ...
        setRunnerAccount(null);
        pushActivity(
            `${clientName} was noted as needing updated ${providerName} credentials. ${completeMessage}`,
            'success',
        );
        setStatus(
            `${clientName} needs updated ${providerName} credentials. ${completeMessage}`,
            'success',
        );
...
async function continueRunner({ source = 'manual' } = {}) {
    ...
    try {
        ...
        if (!settings.office_token) {
            stopRunner();
            throw new Error(
                'Save a CreditSoft API key first from Extension options.',
            );
        }
        ...
        if (!account?.client?.cuid) {
            if (source === 'manual') {
                pushActivity('Checking CreditSoft for the next report pull.');
                setStatus(
                    'Checking CreditSoft for the next report pull...',
                    'info',
                );
            }
            ...
            if (!account?.client || !account?.provider_account) {
                const emptyMessage = activeQueueEmptyMessage();
                stopRunner();
                setRunnerAccount(null);
                setStatus(emptyMessage, 'info');
                pushActivity(emptyMessage, 'warn');
                return;
            }
            ...
            if (source === 'manual') {
                pushActivity(
                    `Loaded ${clientNameFromAccount(account)} for a ${providerLabel(account.provider_account.provider_key)} report pull.`,
                    'success',
                );
            }
        } else {
        ...
        const readyResult = await advanceToCaptureReady(account);
        throwIfCompanionStopped();

        if (readyResult?.credentialProblem) {
            await markProviderInvalidCredentialsAndAdvance(
                settings,
                account,
                readyResult.credentialProblem,
            );
            return;
        }

        const ready = readyResult === true;
        ...
        if (!ready) {
            setStatus(
                `Updating ${providerLabel(account.provider_account.provider_key)} reports for ${clientNameFromAccount(account)}...`,
                'info',
            );
            return;
        }
        ...
        if (
            ...
            if (pendingStep) {
                const pendingUrl = providerStepUrl(
                    toText(account?.provider_account?.provider_key),
                    pendingStep,
                );
                ...
                if (pendingUrl) {
                    pushActivity(
                        `Opening the next page for ${clientNameFromAccount(account)}.`,
                        'info',
                    );
                    setStatus(
                        `Opening the next page for ${clientNameFromAccount(account)}...`,
        ...
        pushActivity(
            `Reading the current page for ${clientNameFromAccount(account)}.`,
        );
        ...
        setStatus(
            `Sending ${clientNameFromAccount(account)} to CreditSoft...`,
            'info',
        );
        ...
        const importedClientName =
            result?.data?.client?.display_name ??
            clientNameFromAccount(account);
        const capturedStep =
            currentStep ??
            detectCaptureStep({
                url: capture.url,
        ...
        if (pendingStep) {
            ...
            if (pendingUrl) {
                pushActivity(
                    `Continuing with ${importedClientName} on the next page.`,
                    'info',
                );
                setStatus(
                    `Continuing with ${importedClientName} on the next page...`,
        ...
        if (logoutUrl) {
            pushActivity(
                `Logging out of ${providerLabel(toText(account?.provider_account?.provider_key))} for ${importedClientName}.`,
                'info',
            );
            setStatus(
                `Logging out of ${providerLabel(toText(account?.provider_account?.provider_key))}...`,
                'info',
            );
            ...
            if (!nextAccount?.client || !nextAccount?.provider_account) {
                ...
                stopRunner();
                setRunnerAccount(null);
                pushActivity(completeMessage, 'success');
                setStatus(completeMessage, 'success');
                return;
            }
        ...
        if (isCompanionStopError(error)) {
            pushActivity(
                'Stopped. No more profiles or reports will open from this run.',
                'warn',
            );
            setStatus('Stopped. Start a new run when ready.', 'warn');
            return;
        }
    ...
    try {
        ...
        if (!quiet) {
            pushActivity(
                error instanceof Error
                    ? error.message
                    : 'Could not verify the CreditSoft API key.',
                'error',
            );
            setStatus(
                error instanceof Error
...
function buildPayload(capture, settings) {
    return {
        ...
        provider_key:
            nextReadyAccount?.provider_account?.provider_key ??
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'CreditSoft rejected the capture.',
            ),
        );
    }
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'CreditSoft could not update the provider status.',
            ),
        );
    }
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'CreditSoft rejected the DisputeFox client sync.',
            ),
        );
    }
...
function safePulseDocumentFilename(documentRecord, response, sourceUrl = '') {
    ...
    const fromUrl = (() => {
        try {
            ...
            const filename = decodeURIComponent(
                file || url.pathname.split('/').filter(Boolean).pop() || '',
            );
    ...
    const candidate = String(headerName || fromUrl || fallback)
        .replace(/[\\/:*?"<>|]+/g, '-')
        .trim();
...
async function fetchPulseDocumentFile(documentRecord) {
    ...
    for (const url of candidateUrls) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `DisputeFox returned HTTP ${response.status} for the document download.`,
                );
            }
            ...
            if (contentType.includes('text/html') && blob.size < 1024 * 1024) {
                throw new Error(
                    'DisputeFox returned a page instead of the document file. Open the document drawer and try again.',
                );
            }
            ...
            return {
                blob,
                fileName: safePulseDocumentFilename(
                    documentRecord,
                    response,
                    url,
                ),
    ...
    if (tinyPreviewSeen) {
        throw new Error(
            'DisputeFox only returned thumbnail-sized document images. Open the document drawer and try again.',
        );
    }
...
async function postClientDocument(
    clientCuid,
    documentRecord,
    filePayload,
    settings,
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'CreditSoft rejected the DisputeFox document upload.',
            ),
        );
    }
...
async function uploadPulseDocumentsForClient(
    documents,
    clientCuid,
    settings,
    capture,
    ...
    const records = (Array.isArray(documents) ? documents : [])
        .filter(
            (documentRecord) =>
                documentRecord?.download_url || documentRecord?.preview_url,
        )
        .slice(0, 80);
    const stats = {
        ...
        try {
            ...
            await postClientDocument(
                clientCuid,
                {
                    ...
                    file_size:
                        filePayload.blob?.size || documentRecord.file_size || 0,
                },
                filePayload,
                settings,
                capture,
            ...
            stats.last_error =
                error instanceof Error
                    ? error.message.replace(/\bPulse\b/g, 'DisputeFox')
                    : 'Could not import a DisputeFox document.';
        }
    }
    ...
    if (!response.ok) {
        throw new Error(
            apiErrorMessage(
                parsed,
                response.status,
                'CreditSoft rejected the automation discovery.',
            ),
        );
    }
...
async function syncAutomationDiscovery(capture, settings) {
    ...
    const message = `Saved ${count} Automation ${noun} to CreditSoft OPS (${created} new, ${updated} updated).`;
...
async function syncDisputeFoxRecordList(capture = null, options = {}) {
    ...
    if (!featureFlags.client_sync) {
        throw new Error(
            'This CreditSoft API key does not have browser companion client sync enabled.',
        );
    }
    ...
    if (rowCount < 1) {
        throw new Error(
            'This DisputeFox page did not expose any list rows to import.',
        );
    }

    pushActivity(`Sending ${rowCount} DisputeFox list rows into CreditSoft.`);
    setStatus(
        `Sending ${rowCount} DisputeFox list rows into CreditSoft...`,
        'info',
    );
    ...
    const message = (
        result?.message || `Imported ${rowCount} DisputeFox ${listKind} rows.`
...
async function syncDisputeFoxProfile() {
    ...
    if (
        ...
        (profile.list_records?.length || 0) > 0
    ) {
    ...
    if (!hasProfile) {
        ...
        const listHint =
            profile.page_kind === 'record-list'
                ? 'This is a list page. Press Import current page to import the visible rows, or open a client/lead row to import one full profile.'
    ...
    const providerAccounts = Array.isArray(
    ...
    if (client?.cuid && documents.length > 0) {
        pushActivity(
            `Found ${documents.length} DisputeFox document records for ${displayName}.`,
            'info',
        );
        setStatus(
            `Found ${documents.length} DisputeFox document records. Importing files when DisputeFox allows it...`,
            'info',
        );
        const uploadStats = await uploadPulseDocumentsForClient(
            documents,
            client.cuid,
            settings,
            capture,
    ...
    pushActivity(
        `Synced ${displayName} from DisputeFox.${documentMessage}${providerMessage}`,
        'success',
    );
    setStatus(
        `Synced ${displayName} from DisputeFox.${documentMessage}${providerMessage}`,
...
async function resolvePulseSessionPrompt() {
    ...
    const result = await executeOnActiveTab(() => {
        ...
        const continueControl = controls.find((element) => {
            ...
            return (
                label.includes('continue and logout') ||
    ...
    if (result?.handled) {
        pushActivity(
            'DisputeFox asked to end the other session. Continuing with this browser.',
            'success',
        );
        setStatus(
            'DisputeFox asked to end the other session. Continuing with this browser...',
    ...
    if (result?.blocked) {
        pushActivity(
            'DisputeFox is blocking the page with a session prompt. Click Continue and Logout other sessions, then try again.',
            'warn',
        );
        setStatus(
            'DisputeFox is blocking the page with a session prompt. Click Continue and Logout other sessions, then try again.',
...
async function openPulseClientsLane() {
    return await executeOnActiveTab(() => {
        ...
        const clientsTrigger =
            document.querySelector('#clientshrefTab') ||
        ...
        return {
            clicked: true,
            label:
                clientsTrigger.textContent?.trim() ||
                clientsTrigger.getAttribute('href') ||
                'Clients',
        };
    });
}
...
async function openPulseVisibleRecord() {
    return await executeOnActiveTab(() => {
        ...
        const table =
            document.querySelector('#clientsORleadsDatatable') ||
            document.querySelector('table.dataTable');
        const selected = table
            ?.querySelector('tbody input.client-list-check:checked')
            ?.closest('tr');
        const row =
            selected ||
            Array.from(table?.querySelectorAll('tbody tr') || []).find(
                (candidate) =>
                    isVisible(candidate) &&
        ...
        const name =
            row.querySelector('.client-grid-name')?.textContent?.trim() ||
...
async function importPulseMigrationLane(lane, settings) {
    ...
    if (isPulseLoginUrl(activePageContext.url)) {
        pushActivity(
            'DisputeFox is on the login screen. Log in, then press the import action again.',
            'warn',
        );
        setStatus(
            'DisputeFox is on the login screen. Log in, then press the import action again.',
    ...
    const expanded = await expandPulseListRows().catch(() => ({
        expanded: false,
    }));

    if (expanded?.expanded) {
        pushActivity(
            `Expanded DisputeFox ${lane.label} list to ${expanded.label || expanded.value}.`,
            'info',
        );
        await delay(2200);
        throwIfCompanionStopped();
    ...
    if (captureLooksLikeAutomationDiscovery(capture)) {
        ...
        const count =
            result?.data?.count || result?.data?.discoveries?.length || 0;
        ...
        return {
            ...
            summary:
                count > 0
                    ? `${lane.label}: ${count} workflows`
                    : `${lane.label}: workflows saved`,
        };
    }
    ...
    if (
        capture.structured_customer?.page_kind === 'record-list' &&
        rowCount > 0
    ) {
        const result = await syncDisputeFoxRecordList(capture, {
            advance: false,
        });
        const data = result?.data || {};
        const saved =
...
async function runPulseMigration(
    ...
    try {
        ...
        for (const lane of lanes) {
            ...
            if (
                pulseMigrationLaneForUrl(activePageContext.url)?.key !==
                lane.key
            ) {
...
function pulseProfileTargetsFromCapture(capture) {
    ...
    return records
        .map((record, index) => {
            ...
            const name =
                pulseRecordValue(record, ['Name', 'Client', 'Client Name']) ||
        ...
        .filter(
            (target) =>
                target.url && target.url.includes('customer_dashboard.jsp'),
        )
        .filter((target) => {
...
async function expandPulseListRows() {
    return await executeOnActiveTab(() => {
        const selects = Array.from(
            document.querySelectorAll(
                'select#lengthDrop, select[name*="length"], select[aria-controls]',
            ),
        );
        ...
        for (const select of selects) {
            ...
            const options = Array.from(select.options)
                .map((option) => ({
                    ...
                    isAll:
                        option.classList.contains('all_client') ||
                        (option.textContent || '')
                            .toLowerCase()
                            .includes('all'),
                }))
            ...
            const target =
                options.find((option) => option.isAll) ||
...
async function processPulseProfilesForLane(lane, settings, seenUrls) {
    ...
    if (pulseMigrationLaneForUrl(activePageContext.url)?.key !== lane.key) {
        pushActivity(
            `Opening DisputeFox ${lane.label} to collect profile links.`,
            'info',
        );
        setStatus(`Opening DisputeFox ${lane.label}...`, 'info');
        await navigateActiveTab(lane.url);
    ...
    if (rows > 0) {
        await syncDisputeFoxRecordList(listCapture, { advance: false }).catch(
            (error) => {
                pushActivity(
                    error instanceof Error
                        ? error.message.replace(/\bPulse\b/g, 'DisputeFox')
                        : `Could not save DisputeFox ${lane.label} list rows first.`,
                    'warn',
                );
            },
        );
    ...
    const targets = pulseProfileTargetsFromCapture(listCapture).filter(
        (target) => {
            const key = target.url.toLowerCase();
    ...
    if (targets.length < 1) {
        pushActivity(
            `No profile links were visible on DisputeFox ${lane.label}.`,
            'warn',
        );
    ...
    for (const [index, target] of cappedTargets.entries()) {
        throwIfCompanionStopped();
        pushActivity(
            `Opening ${lane.label} profile ${index + 1}/${cappedTargets.length}: ${target.name}.`,
            'info',
        );
        setStatus(
            `Importing ${target.name} (${index + 1}/${cappedTargets.length})...`,
            'info',
        );
    ...
    if (targets.length > cappedTargets.length) {
        pushActivity(
            `Imported the first ${cappedTargets.length} ${lane.label} profiles. ${targets.length - cappedTargets.length} remain visible for another pass.`,
            'warn',
        );
    }
...
async function runPulseProfileProcessing() {
    ...
    try {
        ...
        for (const lane of PULSE_MIGRATION_LANES.filter(
            (migrationLane) =>
                migrationLane.key !== 'invoices' &&
                !PULSE_PROFILE_PROCESS_LANES.some(
                    (profileLane) => profileLane.key === migrationLane.key,
                ),
        )) {
            throwIfCompanionStopped();
            await syncActiveContext({ loadQueue: false, quiet: true });
            ...
            if (
                ...
                pushActivity(
                    `Opening DisputeFox ${lane.label} for supporting records.`,
                    'info',
                );
                setStatus(`Opening DisputeFox ${lane.label}...`, 'info');
                await navigateActiveTab(lane.url);
        ...
        const failureSummary =
            failedProfiles.length > 0
                ? ` Failed: ${failedProfiles
                      ...
                      .join(
                          '; ',
                      )}${failedProfiles.length > 5 ? `; plus ${failedProfiles.length - 5} more` : ''}.`
                : '';
        ...
        const supportingSummary =
            supportingSummaries.length > 0
                ? ` Supporting lists: ${supportingSummaries.join(' | ')}.`
                : ' Supporting lists checked; no affiliate or automation rows were imported.';

        finishPulseMigrationMode(
            totalProcessed > 0
                ? `DisputeFox profile import complete. ${summaries.join(' | ')}. DisputeFox data and documents were staged and downloaded where the browser session allowed file access.${supportingSummary}${totalFailed > 0 ? ` ${totalFailed} failed.` : ''}${failureSummary}`
...
async function handleDisputeFoxGo() {
    ...
    if (!isDisputeFoxPulseUrl(activePageContext.url)) {
        ...
        setStatus(
            'Opened the DisputeFox login page. Log in, then choose an import action.',
            'info',
        );
    ...
    const lane = pulseMigrationLaneForUrl(activePageContext.url);

    if (lane?.key && lane.key !== 'clients') {
        await runPulseMigration([lane], `${lane.label} import`);
...
async function resolveApiBaseUrl(settings) {
    ...
    throw new Error(
        'Could not auto-detect the local CreditSoft API. It tries the local router first, then localhost port 80.',
    );
}
...
elements.integrationMenuToggle?.addEventListener('click', () => {
    ...
    const message = integrationMenuOpen
        ? 'Choose a source. SmartCredit and IdentityIQ use Pull next report. Legacy CRM systems use import lanes.'
        : 'Source menu closed.';
...
elements.openDisputeFoxImport?.addEventListener('click', async () => {
    ...
    try {
        ...
        const message =
            error instanceof Error
                ? error.message
                : 'Could not open the DisputeFox login page.';
        pushActivity(message, isCompanionStopError(error) ? 'warn' : 'error');
...
async function runDisputeFoxListImportFromButton(
    button,
    lanes,
    activeLabel,
    restingLabel,
    ...
    try {
        ...
        const message = isCompanionStopError(error)
            ...
            : error instanceof Error
              ? error.message.replace(/\bPulse\b/g, 'DisputeFox')
              : 'Could not import DisputeFox lists.';
        pushActivity(message, isCompanionStopError(error) ? 'warn' : 'error');
        setStatus(message, isCompanionStopError(error) ? 'warn' : 'error');
    } finally {
...
elements.importDisputeFoxLeads?.addEventListener('click', async () => {
    await runDisputeFoxListImportFromButton(
        elements.importDisputeFoxLeads,
        PULSE_MIGRATION_LANES.filter((lane) => lane.key === 'leads'),
        'Leads import',
        'Leads',
    );
});
...
elements.importDisputeFoxInvoices?.addEventListener('click', async () => {
    await runDisputeFoxListImportFromButton(
        elements.importDisputeFoxInvoices,
        PULSE_MIGRATION_LANES.filter((lane) => lane.key === 'invoices'),
...
elements.importDisputeFoxAffiliates?.addEventListener('click', async () => {
    await runDisputeFoxListImportFromButton(
        elements.importDisputeFoxAffiliates,
        PULSE_MIGRATION_LANES.filter((lane) => lane.key === 'affiliates'),
        'Affiliates import',
        'Affiliates',
    );
});
...
elements.importDisputeFoxAutomation?.addEventListener('click', async () => {
    await runDisputeFoxListImportFromButton(
        elements.importDisputeFoxAutomation,
        PULSE_MIGRATION_LANES.filter((lane) => lane.key === 'automation'),
        'Automation import',
        'Automation',
    );
});
...
elements.processPulseProfiles?.addEventListener('click', async () => {
    try {
        ...
        const message = isCompanionStopError(error)
            ? 'Stopped. No more DisputeFox profiles will open from this run.'
            : error instanceof Error
              ? error.message.replace(/\bPulse\b/g, 'DisputeFox')
              : 'Could not import DisputeFox profiles.';
        pushActivity(message, isCompanionStopError(error) ? 'warn' : 'error');
        setStatus(message, isCompanionStopError(error) ? 'warn' : 'error');
    } finally {
        elements.processPulseProfiles.textContent = 'Import client profiles';
        syncFeatureVisibility();
    }
});

elements.stopRun?.addEventListener('click', () => {
    requestCompanionStop();
});

elements.recoverySweep?.addEventListener('click', async () => {
    try {
        await syncActiveContext({ loadQueue: false, quiet: true });

        const confirmed = window.confirm(
            'Run a recovery sweep for 90+ day terminated/canceled/graduated or payment-reactivation candidates? Fired clients and invalid credentials stay excluded.',
        );
        ...
        resetActivity(
            'Recovery sweep started. CreditSoft will only try old recovery candidates from the explicit sweep lane.',
            'warn',
        );
        setStatus(
            'Checking 90+ day recovery candidates. Fired clients and invalid credentials stay excluded.',
        ...
        await continueRunner({ source: 'recovery' });
    } catch (error) {
        const message = isCompanionStopError(error)
            ? 'Stopped. No more recovery candidates will open from this run.'
            : error instanceof Error
              ? error.message
              : 'Could not start the recovery sweep.';
        pushActivity(message, isCompanionStopError(error) ? 'warn' : 'error');
        setStatus(message, isCompanionStopError(error) ? 'warn' : 'error');
    }
});
...
elements.closeDisputeFoxImport?.addEventListener('click', () => {
    finishPulseMigrationMode(
        'DisputeFox import is off. Companion is back to provider report pulls.',
    );
});
...
elements.syncDisputeFoxProfile?.addEventListener('click', async () => {
    try {
        ...
        pushActivity(
            error instanceof Error
                ? error.message
                : 'Could not sync DisputeFox client data.',
            'error',
        );
        setStatus(
            error instanceof Error
...
elements.goCapture?.addEventListener('click', async () => {
    try {
        ...
        const message = isCompanionStopError(error)
            ? 'Stopped. No more provider report pages will open from this run.'
            : error instanceof Error
              ? error.message
              : 'Could not pull a report or import from this page.';
        pushActivity(message, isCompanionStopError(error) ? 'warn' : 'error');
        setStatus(message, isCompanionStopError(error) ? 'warn' : 'error');
    }
});
