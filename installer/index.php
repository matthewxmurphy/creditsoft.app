...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'test_db_connection') {
        ...
        $ourTables = array_filter($existingTables, fn($t) => strpos($t, $prefix) === 0);
    ...
    if ($action === 'install_database') {
        ...
        if ($isUpdate) {
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "settings`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "activity_log`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "tasks`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "knowledge_base`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "sops`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "drip_queue`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "drip_campaigns`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "account_errors`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "dispute_templates`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "disputes`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "error_types`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "report_accounts`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "credit_reports`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "clients`");
            $conn->query("DROP TABLE IF EXISTS `" . $prefix . "users`");
        }
        ...
        if ($importError && strpos($importError, 'already exists') === false) {
            $conn->close();
            respond(false, 'Database import error: ' . $importError);
        }

        $conn->close();
        $_SESSION['db_installed'] = true;

        respond(true, $isUpdate ? 'Database updated!' : 'Database installed!', ['updated' => $isUpdate]);
    }
...
return [
    ...
    // App
    'app.baseURL' => '" . (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . "/',
...
";
        ...
        $conn->query("UPDATE " . $prefix . "users SET email = '" . $conn->real_escape_string($admin_email) . "', password_hash = '" . $conn->real_escape_string($password_hash) . "' WHERE id = 1");

        $conn->query("INSERT INTO " . $prefix . "settings (`key`, `value`) VALUES ('company_logo', '$logo') ON DUPLICATE KEY UPDATE `value` = '$logo'");
        ...
        session_destroy();
        respond(true, 'Installation complete!', ['redirect' => '/dashboard']);
    }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .progress-step.active { background: rgba(0, 212, 255, 0.2); }
        .progress-step.complete::before { transform: scaleX(1); }
        .progress-step.complete { background: rgba(0, 255, 136, 0.2); }
            ...
            <div class="step" data-step="5">
                ...
                <div style="background: #f8fafc; padding: 20px; border-radius: 12px; margin: 20px 0;">
                    ...
                    <ul style="font-size: 14px; color: var(--text); padding-left: 20px;">
                        <li>Login at <strong>/dashboard</strong></li>
