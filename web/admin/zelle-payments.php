...
/**
 * Zelle payment automation for the public CreditSoft admin.
 *
...
function cs_site_zelle_payment_notices(): array
{
    ...
    foreach (cs_site_zelle_checkout_paths() as $path) {
        foreach (cs_site_zelle_read_jsonl($path) as $row) {
            $rows[] = $row;
        }
    }
...
function cs_site_zelle_discounted_amount(float $base, int $discountPercent = 10): float
{
...
function cs_site_zelle_expected_amount_catalog(): array
{
    ...
    $rows = [];
    $appendRow = static function (string $publicPlanKey, string $planKey, string $planName, string $billing, float $base, array $addonKeys = []) use (&$rows): void {
        $base = round($base, 2);
        ...
        $zelle = cs_site_zelle_discounted_amount($base);
        $rows[] = [
...
function cs_site_zelle_enterprise_with_node_catalog_row(string $billing): array
{
    ...
    $clusterBase = round((float) ($cluster[$billing] ?? 19.95), 2);
    $base = round((float) ($enterprise['base_amount'] ?? 99.95) + $clusterBase, 2);
    ...
    return [
        ...
        'addons' => ['cluster'],
        'base_amount' => $base,
        'expected_amount' => cs_site_zelle_discounted_amount($base),
        'discount_percent' => 10,
    ];
}
...
function cs_site_zelle_lead_by_email(PDO $pdo, string $email): ?array
{
    ...
    $stmt = $pdo->prepare('SELECT * FROM leads WHERE LOWER(email) = LOWER(?) LIMIT 1');
    $stmt->execute([$email]);
...
function cs_site_zelle_lead_by_name(PDO $pdo, string $name): ?array
{
    ...
    $stmt = $pdo->prepare('SELECT * FROM leads WHERE LOWER(name) LIKE LOWER(?) ORDER BY created_at DESC LIMIT 1');
    $stmt->execute(['%'.$name.'%']);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
function cs_site_zelle_license_by_email(PDO $pdo, string $email): ?array
{
    ...
    $stmt = $pdo->prepare('SELECT * FROM licenses WHERE LOWER(customer_email) = LOWER(?) ORDER BY created_at DESC LIMIT 1');
    $stmt->execute([$email]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
function cs_site_zelle_license_by_key(PDO $pdo, string $licenseKey): ?array
{
    ...
    $stmt = $pdo->prepare("SELECT * FROM licenses WHERE REPLACE(REPLACE(UPPER(license_key), '-', ''), ' ', '') = ? LIMIT 1");
...
function cs_site_zelle_issue_or_extend_license(PDO $pdo, array $customer, string $planKey, string $billing, ?float $amount, array $paymentContext = []): array
{
    ...
    if ($subscriptionId > 0) {
        ...
        $subscription->execute([$billing, $amount, $expiresAt, $subscriptionId]);
    } else {
        $subscription = $pdo->prepare(
        ...
        $subscription->execute([$licenseId, $billing, $amount, $expiresAt]);
    }

    $transactionId = cs_site_zelle_clean_text((string) ($paymentContext['transaction_id'] ?? ''), 255);
    $details = 'Zelle payment matched and license '.$action.'.';
    if ($transactionId !== '') {
    ...
    if ($amount !== null) {
        $details .= ' Paid: '.cs_site_zelle_money($amount).'.';
    }
...
function cs_site_zelle_branded_email(array $content): string
{
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . $title . '</title></head>'
        ...
        . '<p style="margin:18px 0 0;color:#94a3b8;font-size:12px;line-height:1.6;">CreditSoft App<br>z@creditsoft.app for Zelle payments | hello@creditsoft.app for support</p>'
        . '</td></tr>'
        . '</table>'
        . $trackingHtml
        . '</td></tr></table>'
...
function cs_site_zelle_send_balance_due_email(array $customer, array $expectedPayment, string $transactionId = '', string $trackingToken = ''): bool
{
    ...
    $lines = [
        $greeting,
        '',
        'Thank you. CreditSoft received your Zelle payment and matched it to your account email.',
        '',
    ...
    $lines[] = 'Please send the remaining balance by Zelle to z@creditsoft.app and use this email address as the memo: '.$email;
    ...
    $html = cs_site_zelle_branded_email([
        'tone' => 'warning',
        'preheader' => 'CreditSoft received your Zelle payment. A remaining balance is still needed.',
        'eyebrow' => 'Balance due',
        'title' => 'Thank you - we received your payment.',
        'intro' => $greeting . ' CreditSoft matched your Zelle payment to your account email, but the amount came in below the total for this order.',
        'paragraphs' => array_values(array_filter([
            ...
            'Please send the remaining balance by Zelle to z@creditsoft.app and use this email address as the memo: '.$email,
...
function cs_site_zelle_process_message(PDO $pdo, array $message, bool $force = false): array
{
    ...
    if (
        ...
        return [
            'status' => 'ignored',
            'message' => 'Ignored non-payment sender. Only trusted Chase/Zelle sender domains are processed.',
    ...
    if (! cs_site_zelle_subject_is_expected($subject)) {
        return [
            'status' => 'ignored',
            'message' => 'Ignored message because the subject did not match "' . cs_site_zelle_expected_subject() . '".',
            'from_email' => cs_site_zelle_normalize_email((string) ($message['from_email'] ?? '')),
            'subject' => $subject,
        ];
    }
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO zelle_payment_messages (
            mailbox, message_uid, message_id, transaction_id, received_at, from_name, from_email, subject, body_excerpt,
...
function cs_site_zelle_force_issue_message(PDO $pdo, int $messageId, string $planKey = 'enterprise', string $billing = 'monthly', ?float $amountOverride = null): array
{
    ...
    if (empty($headerTrust['trusted']) && ! cs_site_zelle_domain_is_trusted(cs_site_zelle_email_domain((string) ($row['from_email'] ?? '')), cs_site_zelle_trusted_domains())) {
        return ['success' => false, 'message' => 'Only trusted Chase/Zelle payment messages can be force-issued.'];
    }
    ...
    $amount = $amountOverride !== null
        ...
        : (is_numeric($catalog['expected_amount'] ?? null) ? round((float) $catalog['expected_amount'], 2) : null);
    ...
    $license = cs_site_zelle_issue_or_extend_license($pdo, $customer, $planKey, $billing, $amount, [
        'transaction_id' => $transactionId,
    ...
    $metadata['owner_override'] = [
        'at' => gmdate('c'),
        'reason' => 'Trusted payment force-issued as a paid Enterprise license test.',
    ...
    $update = $pdo->prepare(
        "UPDATE zelle_payment_messages
         SET amount = ?,
             ...
             status = 'processed',
             payment_status = 'paid',
             match_type = 'owner_override_paid',
             sender_email = ?,
             license_id = ?,
             license_key = ?,
             onboarding_url = ?,
...
function cs_site_zelle_send_payment_ticket_confirmation(array $ticket): void
{
    ...
    $plain = [
        ...
        '',
        'Please remember to always include your CreditSoft account email in the Zelle memo in the future so your license can match automatically.',
        '',
        'CreditSoft',
    ];
    $html = cs_site_zelle_branded_email([
        ...
        'intro' => $greeting . ' CreditSoft has your payment help request. This gives support the details needed to match a Zelle payment that did not automatically connect to a license.',
        'paragraphs' => [
        ...
        'secondary_url' => cs_site_zelle_absolute_public_url('/client-portal', ['email' => $email]),
        'secondary_label' => 'open the billing history',
        'footer_note' => 'Support will compare the ticket details against Zelle mail, checkout notices, and license records.',
    ]);
