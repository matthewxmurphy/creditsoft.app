...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'save_pricing') {
        $input = [
            'note' => $_POST['note'] ?? '',
            'manual_payment_discount_percent' => $_POST['manual_payment_discount_percent'] ?? creditsoft_site_manual_payment_discount_percent(),
            'plans' => $_POST['plans'] ?? [],
            'addons' => $_POST['addons'] ?? [],
        ];
    ...
    if ($action === 'archive_customer') {
        ...
        $result = $pdo instanceof PDO
            ? cs_site_admin_archive_customer(
                $pdo,
                (string) ($_POST['customer_email'] ?? ''),
        ...
        if (! empty($result['success'])) {
            $counts = is_array($result['row_counts'] ?? null) ? $result['row_counts'] : [];
            $summary = implode(', ', array_map(
                static fn (string $key, int $count): string => str_replace('_', ' ', $key) . ': ' . $count,
                array_keys($counts),
                array_map('intval', $counts)
            ));
            cs_site_admin_flash('success', 'Customer archived with reason. Archive #' . (int) ($result['archive_id'] ?? 0) . ($summary !== '' ? ' Updated ' . $summary . '.' : '.'));
        } else {
            cs_site_admin_flash('error', (string) ($result['message'] ?? 'Could not archive that customer yet.'));
        }
    ...
    if ($action === 'run_zelle_payment_checker') {
        ...
        if (! empty($result['success'])) {
            cs_site_admin_flash(
                'success',
                'Payment inbox checked. Fetched ' . (int) ($result['fetched'] ?? 0) . '. Processed ' . (int) ($result['processed'] ?? 0) . ', review ' . (int) ($result['needs_review'] ?? 0) . ', short pays ' . (int) ($result['balance_due'] ?? 0) . '.'
            );
        } else {
...
$panelHero = [
    ...
    'payments' => [
        'title' => 'Payment inbox.',
        'copy' => 'Review Cash App, Zelle, checkout notices, and license actions.',
    ],
    'customers' => [
...
function admin_date(?string $value, string $fallback = '—'): string
{
    return admin_format_datetime($value, $fallback);
}

function admin_json($value): string
{
    $encoded = json_encode($value, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT);

    return is_string($encoded) ? $encoded : '{}';
}

function admin_payment_sender_name(array $message): string
{
    $name = cs_site_zelle_clean_sender_display_name((string) ($message['sender_name'] ?? $message['from_name'] ?? ''));

    return $name !== '' ? $name : 'Unknown sender';
}

function admin_payment_amount_context(array $message): array
{
    $expectedAmount = isset($message['expected_amount']) && is_numeric($message['expected_amount']) ? round((float) $message['expected_amount'], 2) : null;
    $planAmount = null;
    $discountAmount = null;
    $discountPercent = null;
    $messagePlanKey = (string) ($message['plan_key'] ?? '');
    $messageBilling = (string) ($message['billing'] ?? '');

    foreach (cs_site_zelle_expected_amount_catalog() as $row) {
        if (
            $messagePlanKey !== ''
            && $messageBilling !== ''
            && (string) ($row['plan_key'] ?? '') === $messagePlanKey
            && (string) ($row['billing'] ?? '') === $messageBilling
            && (
                $expectedAmount === null
                || (isset($row['expected_amount']) && is_numeric($row['expected_amount']) && abs((float) $row['expected_amount'] - $expectedAmount) <= 0.02)
                || (isset($row['base_amount']) && is_numeric($row['base_amount']) && abs((float) $row['base_amount'] - $expectedAmount) <= 0.02)
            )
        ) {
            $planAmount = isset($row['base_amount']) && is_numeric($row['base_amount']) ? round((float) $row['base_amount'], 2) : null;
            $discountPercent = isset($row['discount_percent']) && is_numeric($row['discount_percent']) ? (int) $row['discount_percent'] : null;
            break;
        }
    }
    ...
    if ($planAmount === null) {
        ...
        $metaPlanMatches = $messagePlanKey === ''
            || (string) ($expectedPaymentMeta['plan_key'] ?? '') === ''
            || (string) ($expectedPaymentMeta['plan_key'] ?? '') === $messagePlanKey;
        $metaBillingMatches = $messageBilling === ''
            || (string) ($expectedPaymentMeta['billing'] ?? '') === ''
            || (string) ($expectedPaymentMeta['billing'] ?? '') === $messageBilling;

        if ($metaPlanMatches && $metaBillingMatches) {
            $planAmount = isset($expectedPaymentMeta['base_amount']) && is_numeric($expectedPaymentMeta['base_amount']) ? round((float) $expectedPaymentMeta['base_amount'], 2) : null;
            $discountPercent = isset($expectedPaymentMeta['discount_percent']) && is_numeric($expectedPaymentMeta['discount_percent']) ? (int) $expectedPaymentMeta['discount_percent'] : $discountPercent;
        }
    }

    if ($planAmount !== null && $expectedAmount !== null && $planAmount > ($expectedAmount + 0.02)) {
        $discountAmount = round($planAmount - $expectedAmount, 2);
    }
...
function admin_payment_sales_chart_payload(array $summary): array
{
    $daily = is_array($summary['daily'] ?? null) ? $summary['daily'] : [];
    $providers = is_array($summary['provider_rows'] ?? null) ? $summary['provider_rows'] : [];
    $plans = is_array($summary['by_plan'] ?? null) ? $summary['by_plan'] : [];
    $firstSaleIndex = null;

    foreach (array_values($daily) as $index => $row) {
        $gross = (float) ($row['amount'] ?? 0);
        $zelle = (float) ($row['zelle'] ?? 0);
        $cashApp = (float) ($row['cash_app'] ?? 0);

        if ($gross > 0 || $zelle > 0 || $cashApp > 0) {
            $firstSaleIndex = $index;
            break;
        }
    }

    if ($firstSaleIndex !== null) {
        $daily = array_slice(array_values($daily), $firstSaleIndex);
    }

    return [
        'windowDays' => (int) ($summary['window_days'] ?? 30),
        'feePercent' => round((float) ($summary['cash_app_fee_percent'] ?? 0), 2),
        'totals' => [
            'gross' => round((float) ($summary['total'] ?? 0), 2),
            'net' => round((float) ($summary['net_total'] ?? ($summary['total'] ?? 0)), 2),
            'fees' => round((float) ($summary['fee_total'] ?? 0), 2),
            'count' => (int) ($summary['count'] ?? 0),
            'average' => round((float) ($summary['average'] ?? 0), 2),
        ],
        'daily' => [
            'labels' => array_values(array_map(static fn (array $row): string => (string) ($row['label'] ?? ''), $daily)),
            'zelle' => array_values(array_map(static fn (array $row): float => round((float) ($row['zelle'] ?? 0), 2), $daily)),
            'cashApp' => array_values(array_map(static fn (array $row): float => round((float) ($row['cash_app'] ?? 0), 2), $daily)),
            'gross' => array_values(array_map(static fn (array $row): float => round((float) ($row['amount'] ?? 0), 2), $daily)),
            'net' => array_values(array_map(static fn (array $row): float => round((float) ($row['net'] ?? ($row['amount'] ?? 0)), 2), $daily)),
            'fees' => array_values(array_map(static fn (array $row): float => round((float) ($row['fees'] ?? 0), 2), $daily)),
        ],
        'providers' => [
            'labels' => array_values(array_map(static fn (array $row): string => (string) ($row['label'] ?? 'Payment'), $providers)),
            'amounts' => array_values(array_map(static fn (array $row): float => round((float) ($row['amount'] ?? 0), 2), $providers)),
            'counts' => array_values(array_map(static fn (array $row): int => (int) ($row['count'] ?? 0), $providers)),
            'fees' => array_values(array_map(static fn (array $row): float => round((float) ($row['fees'] ?? 0), 2), $providers)),
            'net' => array_values(array_map(static fn (array $row): float => round((float) ($row['net'] ?? 0), 2), $providers)),
        ],
        'plans' => [
            'labels' => array_values(array_map(static fn (array $row): string => (string) ($row['label'] ?? 'Unassigned'), $plans)),
            'amounts' => array_values(array_map(static fn (array $row): float => round((float) ($row['amount'] ?? 0), 2), $plans)),
            'net' => array_values(array_map(static fn (array $row): float => round((float) ($row['net'] ?? 0), 2), $plans)),
            'counts' => array_values(array_map(static fn (array $row): int => (int) ($row['count'] ?? 0), $plans)),
        ],
    ];
}
...
function admin_icon(string $name): string
{
    $icons = [
        ...
        'plans' => 'fa-solid fa-tags',
        'content' => 'fa-solid fa-file-lines',
        'licenses' => 'fa-solid fa-key',
        'payments' => 'fa-solid fa-money-check-dollar',
        ...
        'social' => 'fa-brands fa-meta',
        'ops' => 'fa-solid fa-gears',
        'portal' => 'fa-solid fa-door-open',
        'site' => 'fa-solid fa-globe',
        'logout' => 'fa-solid fa-right-from-bracket',
    ];

    return '<i class="' . ($icons[$name] ?? $icons['dashboard']) . ' fa-fw" aria-hidden="true"></i>';
}

function admin_payment_icon(string $name): string
{
    $icons = [
        'mailbox' => 'fa-solid fa-envelope',
        'database' => 'fa-solid fa-database',
        'review' => 'fa-solid fa-triangle-exclamation',
        'balance' => 'fa-solid fa-money-bill-wave',
        'credit' => 'fa-solid fa-circle-arrow-up',
        'ticket' => 'fa-solid fa-ticket-simple',
        'processed' => 'fa-solid fa-circle-check',
        'license' => 'fa-solid fa-id-card',
        'proof' => 'fa-solid fa-file-shield',
    ];

    return '<i class="' . ($icons[$name] ?? $icons['processed']) . ' fa-fw" aria-hidden="true"></i>';
}

function admin_payment_brand_logo(string $brand): string
{
    if ($brand === 'zelle') {
        return '<img class="payment-brand-image is-zelle" src="' . admin_escape(cs_site_public_url('/assets/vendor-logos/zelle.svg')) . '" alt="Zelle">';
    }

    if ($brand === 'cashapp') {
        return '<span class="payment-brand-cashapp-mark" aria-hidden="true"><svg viewBox="0 0 32 32" focusable="false"><path fill-rule="evenodd" clip-rule="evenodd" d="M11.1452 0.0012311C7.93691 0.0012311 6.33155 0.00123113 4.61663 0.534298C2.72442 1.22002 1.23233 2.71212 0.54661 4.60678C0 6.33895 0 7.94184 0 11.1513V20.8351C0 24.0619 0 25.6512 0.533068 27.3797C1.21879 29.2731 2.71088 30.7652 4.60308 31.4509C6.33401 32 7.93691 32 11.1452 32H20.8389C24.0508 32 25.6525 32 27.3797 31.4509C29.2731 30.7652 30.764 29.2731 31.4497 27.3797C31.9963 25.6475 31.9963 24.0446 31.9963 20.8351V11.1649C31.9963 7.95169 31.9963 6.34879 31.4497 4.62033C30.764 2.72689 29.2719 1.2348 27.3797 0.549072C25.6365 0 24.0508 0 20.8229 0H11.1439L11.1452 0.0012311Z" fill="#00D633"/><path d="M17.2927 14.4459C20.6641 15.1683 22.2053 16.565 22.2053 18.925C22.2053 21.8835 19.7962 24.0674 16.0354 24.2986L15.6685 26.0485C15.6326 26.2196 15.4818 26.3421 15.307 26.3421H12.4138C12.1754 26.3421 11.9994 26.1195 12.0545 25.8875L12.5188 23.9305C10.6547 23.4001 9.15468 22.3683 8.28022 21.0948C8.17012 20.9345 8.20665 20.7154 8.36018 20.596L10.3824 19.0232C10.5491 18.8935 10.788 18.9315 10.9112 19.103C11.9827 20.5944 13.6266 21.4776 15.607 21.4776C17.389 21.4776 18.7375 20.6107 18.7375 19.3585C18.7375 18.3952 18.0633 17.9618 15.7996 17.4801C11.8984 16.6614 10.3572 15.2165 10.3572 12.8565C10.3572 10.1183 12.6585 8.04985 16.1297 7.7812L16.5087 5.97378C16.5445 5.80273 16.6954 5.68024 16.8701 5.68024H19.7245C19.96 5.68024 20.1353 5.89764 20.0855 6.12777L19.6467 8.15292C21.1404 8.6127 22.3567 9.43482 23.1138 10.4493C23.2311 10.6066 23.2009 10.8292 23.0492 10.9536L21.2042 12.4665C21.0439 12.598 20.8098 12.5693 20.6795 12.4081C19.7418 11.2483 18.2656 10.5928 16.6665 10.5928C14.8845 10.5928 13.7768 11.3634 13.7768 12.4712C13.7768 13.3863 14.6437 13.8679 17.2927 14.4459Z" fill="white"/></svg></span><span class="payment-brand-text">Cash App</span>';
    }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .rail-brand { width: 42px; height: 42px; display: grid; place-items: center; color: var(--rail-accent); text-decoration: none; }
        .rail-brand img { width: 42px; height: 42px; display: block; object-fit: contain; filter: drop-shadow(0 8px 14px rgba(0,0,0,.28)); }
        .rail-brand svg, .rail-item__icon svg, .rail-item svg { width: 19px; height: 19px; display: block; fill: currentColor; }
        .rail-item i { display: block; width: 1.25em; font-size: 18px; line-height: 1; text-align: center; }
        ...
        .topbar-brand { width: 46px; height: 46px; display: inline-flex; align-items: center; justify-content: center; line-height: 0; color: #111827; text-decoration: none; flex: 0 0 auto; }
        .topbar-brand img { width: 46px; height: 46px; display: block; object-fit: contain; filter: drop-shadow(0 10px 18px rgba(17,24,39,.14)); }
        .topbar-copy { min-width: 0; max-width: 100%; flex: 1 1 auto; }
        .topbar-kicker { font-size: 11px; letter-spacing: .22em; text-transform: uppercase; color: #9ca3af; font-weight: 800; }
        .topbar-title { font-size: 17px; font-weight: 800; line-height: 1.1; overflow-wrap: anywhere; }
        ...
        .section-card { padding: 22px; display: grid; gap: 18px; min-width: 0; }
        ...
        .section-head { display: flex; justify-content: space-between; gap: 12px; align-items: end; min-width: 0; }
        ...
        .lead-list { display: grid; gap: 12px; min-width: 0; }
        .lead-row { display: grid; grid-template-columns: minmax(175px, 1.2fr) minmax(130px, .75fr) minmax(150px, .85fr); gap: 14px; padding: 16px 18px; border: 1px solid var(--line); border-radius: 22px; background: rgba(255,255,255,.88); align-items: start; min-width: 0; }
        .lead-row > * { min-width: 0; }
        .lead-name { font-weight: 800; font-size: 17px; }
        .lead-sub { color: var(--muted); font-size: 13px; margin-top: 4px; line-height: 1.45; overflow-wrap: anywhere; }
        .label-stack { display: grid; gap: 8px; }
        .soft-label { display: inline-flex; align-items: center; gap: 8px; padding: 8px 11px; border-radius: 12px; font-size: 11px; letter-spacing: .12em; text-transform: uppercase; font-weight: 800; border: 1px solid var(--line); background: #f8fafc; color: #6b7280; width: fit-content; max-width: 100%; overflow-wrap: anywhere; }
        ...
        .payment-command { display: grid; grid-template-columns: 1fr; gap: 12px; align-items: start; }
        .payment-command-panel { border-radius: 18px; border: 1px solid rgba(15,23,42,.10); background: linear-gradient(135deg, rgba(255,255,255,.98), rgba(250,248,240,.94)); padding: 14px; display: grid; gap: 12px; align-content: start; }
        .payment-command-panel.is-mailbox { grid-template-columns: minmax(330px, .86fr) minmax(420px, 1fr); align-items: center; padding: 12px 14px; }
        .payment-command-panel.is-analytics { gap: 12px; }
        .payment-command-title { display: flex; align-items: flex-start; gap: 10px; }
        .payment-icon { width: 34px; height: 34px; display: grid; place-items: center; color: #111827; flex: 0 0 auto; }
        .payment-icon svg { width: 28px; height: 28px; display: block; fill: currentColor; }
        .payment-icon i { display: block; font-size: 28px; line-height: 1; }
        .payment-icon.is-zelle { color: #6d1ed4; }
        .payment-icon.is-good { color: #15803d; }
        .payment-icon.is-warn { color: #b45309; }
        .payment-icon.is-muted { color: #475569; }
        ...
        .payment-command-panel.is-mailbox .payment-command-title { align-items: center; }
        .payment-command-panel.is-mailbox .payment-command-title.is-compact { display: none; }
        .payment-command-panel.is-mailbox .payment-title-stack { gap: 3px; }
        .payment-health-list { display: grid; gap: 10px; }
        .payment-command-panel.is-mailbox .payment-health-list { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 18px; }
        ...
        .payment-command-panel.is-mailbox .payment-health-row { grid-template-columns: 38px minmax(0, 1fr) auto; padding-top: 0; padding-left: 0; border-top: 0; border-left: 0; }
        .payment-command-panel.is-mailbox .payment-health-row:first-child { padding-left: 0; border-left: 0; }
        .payment-mini-icon { width: 24px; height: 24px; display: grid; place-items: center; color: #475569; }
        .payment-mini-icon svg { width: 20px; height: 20px; fill: currentColor; }
        .payment-mini-icon i { display: block; font-size: 20px; line-height: 1; }
        .payment-health-copy { display: grid; gap: 2px; }
        .payment-command-panel.is-mailbox .payment-mini-icon { width: 36px; height: 36px; }
        .payment-command-panel.is-mailbox .payment-mini-icon svg { width: 30px; height: 30px; }
        .payment-command-panel.is-mailbox .payment-mini-icon i { font-size: 30px; }
        .payment-health-copy strong { font-size: 13px; }
        .payment-health-copy span { color: var(--muted); font-size: 11px; line-height: 1.35; }
        .payment-command-panel.is-mailbox .payment-health-copy strong { font-size: 15px; line-height: 1.05; }
        .payment-command-panel.is-mailbox .payment-health-copy span { font-size: 12px; line-height: 1.35; }
        .payment-mailbox-meta { grid-column: 1 / -1; margin-top: -4px; color: var(--muted); font-size: 11px; line-height: 1.35; }
        ...
        .payment-metric-grid { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 8px; }
        .payment-metric { border-radius: 15px; border: 1px solid rgba(15,23,42,.08); background: rgba(255,255,255,.86); padding: 9px; display: grid; gap: 6px; min-height: 74px; align-content: start; }
        .payment-metric-top { display: flex; align-items: center; justify-content: space-between; gap: 10px; }
        .payment-metric-top .payment-mini-icon { color: #6d1ed4; }
        .payment-metric.is-alert .payment-mini-icon { color: #b45309; }
        .payment-metric.is-credit .payment-mini-icon { color: #0f766e; }
        .payment-metric.is-good .payment-mini-icon { color: #15803d; }
        .payment-metric-value { font-size: 25px; font-weight: 850; letter-spacing: -.04em; line-height: .9; }
        .payment-metric-label { color: #64748b; font-size: 9px; font-weight: 850; letter-spacing: .10em; text-transform: uppercase; }
        ...
        .payment-message-primary .payment-mini-icon { width: 34px; height: 34px; color: #475569; flex: 0 0 auto; }
        .payment-message-primary .payment-mini-icon svg { width: 28px; height: 28px; }
        .payment-message-primary .payment-mini-icon i { font-size: 28px; }
        .payment-message-card.is-processed .payment-message-primary .payment-mini-icon { color: #15803d; }
        .payment-message-card.is-balance_due .payment-message-primary .payment-mini-icon,
        .payment-message-card.is-needs_review .payment-message-primary .payment-mini-icon { color: #b45309; }
        ...
        .payment-brand-strip { display: flex; align-items: center; justify-content: flex-start; gap: 28px; flex-wrap: wrap; min-width: 0; padding: 4px 0 2px; }
        .payment-brand-card { display: contents; }
        .payment-brand-logo { width: auto; min-width: 0; height: auto; display: inline-flex; align-items: center; justify-content: center; gap: 10px; color: #111827; line-height: 0; }
        .payment-brand-image { display: block; object-fit: contain; }
        .payment-brand-image.is-zelle { width: 126px; max-width: 42vw; max-height: 46px; }
        .payment-brand-cashapp-mark { display: inline-flex; width: 52px; height: 52px; line-height: 0; }
        .payment-brand-cashapp-mark svg { width: 52px; height: 52px; display: block; }
        .payment-brand-text { font-size: 20px; line-height: 1; font-weight: 900; letter-spacing: -.02em; }
        .payment-sales-card { border-top: 1px solid var(--line); padding-top: 12px; display: grid; gap: 10px; }
        .payment-sales-head { display: flex; justify-content: space-between; align-items: end; gap: 14px; }
        .payment-sales-head strong { font-size: 13px; letter-spacing: .12em; text-transform: uppercase; }
        .payment-sales-head span { color: var(--muted); font-size: 11px; }
        .payment-sales-total { font-size: 27px; font-weight: 850; letter-spacing: -.04em; line-height: .95; }
        .payment-sales-bars { height: 92px; display: flex; align-items: end; gap: 4px; padding: 10px 8px 8px; border-radius: 14px; border: 1px solid rgba(15,23,42,.07); background: linear-gradient(180deg, rgba(248,250,252,.92), rgba(255,255,255,.70)); }
        .payment-sales-bar { flex: 1; min-width: 4px; height: var(--bar-h); border-radius: 8px 8px 3px 3px; background: linear-gradient(180deg, #111827, #22c55e); position: relative; }
        .payment-sales-bar.is-empty { opacity: .18; background: #94a3b8; }
        .payment-sales-bar:hover::after { content: attr(data-label); position: absolute; left: 50%; bottom: calc(100% + 8px); transform: translateX(-50%); white-space: nowrap; padding: 5px 7px; border-radius: 8px; background: #111827; color: #fff; font-size: 10px; font-weight: 800; z-index: 2; box-shadow: 0 12px 24px rgba(15,23,42,.20); }
        .payment-sales-legend { display: flex; gap: 10px; flex-wrap: wrap; color: var(--muted); font-size: 11px; }
        .payment-sales-legend span { display: inline-flex; align-items: center; gap: 6px; }
        .payment-sales-legend span::before { content: ''; width: 8px; height: 8px; border-radius: 999px; background: #22c55e; }
        .payment-sales-legend span:nth-child(2)::before { background: #00d632; }
        .payment-sales-legend span:nth-child(3)::before { background: #111827; }
        .payment-analytics-grid { display: grid; grid-template-columns: minmax(0, 1.75fr) minmax(250px, .55fr); gap: 12px; }
        .payment-analytics-grid.is-secondary { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        .payment-chart-card { border-radius: 17px; border: 1px solid rgba(15,23,42,.08); background: rgba(255,255,255,.88); padding: 12px; display: grid; gap: 10px; min-width: 0; }
        .payment-chart-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
        .payment-chart-title { display: grid; gap: 3px; }
        .payment-chart-title strong { font-size: 13px; letter-spacing: .10em; text-transform: uppercase; }
        .payment-chart-title span { color: var(--muted); font-size: 11px; line-height: 1.35; }
        .payment-chart-stat { text-align: right; display: grid; gap: 2px; font-size: 11px; color: var(--muted); }
        .payment-chart-stat strong { color: var(--ink); font-size: 21px; line-height: .95; letter-spacing: -.04em; }
        .payment-chart-wrap { position: relative; height: 260px; min-height: 0; max-height: 260px; overflow: hidden; contain: layout size; }
        .payment-chart-wrap.is-small { height: 190px; max-height: 190px; }
        .payment-chart-wrap canvas { display: block; width: 100% !important; height: 100% !important; max-height: 100% !important; }
        .payment-kpi-row { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 9px; }
        .payment-kpi { border-radius: 14px; border: 1px solid rgba(15,23,42,.08); background: linear-gradient(180deg, rgba(248,250,252,.95), rgba(255,255,255,.86)); padding: 10px; display: grid; gap: 5px; }
        .payment-kpi span { color: #64748b; font-size: 10px; font-weight: 850; letter-spacing: .12em; text-transform: uppercase; }
        .payment-kpi strong { font-size: 22px; letter-spacing: -.04em; line-height: 1; }
        .payment-kpi small { color: var(--muted); font-size: 11px; line-height: 1.25; }
        .payment-chart-empty { display: none; border-radius: 14px; border: 1px dashed rgba(15,23,42,.18); padding: 18px; color: var(--muted); font-size: 13px; line-height: 1.45; }
        .payment-chart-empty.is-visible { display: block; }
        ...
        .license-copy-name strong { text-decoration: underline; text-decoration-color: rgba(15,23,42,.22); text-underline-offset: 4px; }
        .license-copy-name:hover strong { color: var(--accent); text-decoration-color: currentColor; }
        .license-copy-name:focus-visible { outline: 2px solid var(--accent); outline-offset: 4px; border-radius: 8px; }
        .license-copy-name small { color: var(--muted); line-height: 1.4; }
        .license-copy-status { color: #047857 !important; font-weight: 800; }
        .stats-inline { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; }
        .archive-inline-form { display: grid; grid-template-columns: minmax(120px, 1fr) auto; gap: 7px; align-items: center; min-width: 230px; }
        .archive-inline-form .admin-select { min-height: 36px; padding: 7px 9px; font-size: 11px; border-radius: 10px; }
        .archive-inline-form .hero-btn { padding: 8px 10px; border-radius: 10px; font-size: 11px; white-space: nowrap; }
        .hidden { display: none !important; }
        @media (max-width: 1260px) {
            .stats { grid-template-columns: repeat(2, minmax(0, 1fr)); }
            .overview-grid, .seo-page-grid { grid-template-columns: 1fr; }
            .mini-grid, .admin-form-grid.is-three, .stats-inline, .social-form-grid, .social-signal-grid, .social-summary-grid, .social-split-grid, .social-kpi-grid, .payment-command, .payment-message-card, .payment-analytics-grid, .payment-analytics-grid.is-secondary { grid-template-columns: 1fr; }
            .payment-command-panel.is-mailbox { grid-template-columns: 1fr; align-items: start; }
            .payment-command-panel.is-mailbox .payment-brand-strip { justify-content: flex-start; }
            .payment-command-panel.is-mailbox .payment-health-list { grid-template-columns: 1fr; }
            .payment-command-panel.is-mailbox .payment-health-row { padding-top: 8px; padding-left: 0; border-left: 0; border-top: 1px solid var(--line); }
            .payment-command-panel.is-mailbox .payment-health-row:first-child { padding-top: 0; border-top: 0; }
            .payment-metric-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
            .payment-kpi-row { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        ...
        @media (max-width: 860px) {
            ...
            .stats, .detail-grid, .admin-form-grid.is-two, .price-grid, .payment-brand-strip { grid-template-columns: 1fr; }
            .payment-metric-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
            .payment-kpi-row { grid-template-columns: 1fr; }
...
<div class="admin-shell">
    <aside class="rail" aria-label="Site admin rail">
        <a href="<?= admin_url('/') ?>" class="rail-brand" aria-label="CreditSoft site admin home"><img src="<?= admin_escape(public_url('/favicon.svg')) ?>" alt=""></a>
        <nav class="rail-nav">
            ...
            <a class="rail-item<?= admin_nav_active('content', $panel) ?>" href="<?= admin_url('/', ['panel' => 'content']) ?>" title="Site content"><?= admin_icon('content') ?></a>
    ...
    <main class="main">
        <header class="topbar">
            <div class="topbar-left">
                <a class="topbar-brand" href="<?= public_url('/') ?>" aria-label="CreditSoft home"><img src="<?= admin_escape(public_url('/favicon.svg')) ?>" alt=""></a>
        ...
        <section class="content">
                        ...
                        <div class="lead-list">
                            <?php foreach (array_slice($leads, 0, 6) as $lead): ?>
                                <?php
                                    ...
                                    $leadHasCustomerContext = cs_site_admin_lead_has_customer_context($lead);
                                ?>
                                <article class="lead-row">
                                    ...
                                    <div><strong><?= admin_escape(cs_site_admin_client_count_label($lead['client_count'] ?? '', $leadHasCustomerContext)) ?></strong><div class="lead-sub"><?= admin_escape(cs_site_admin_workflow_label($lead['current_workflow'] ?? '', $leadHasCustomerContext)) ?></div></div>
            ...
            <section class="card section-card<?= admin_panel_visible('plans', $panel) ?>">
                ...
                <form method="post" class="admin-form-grid" data-autosave="true">
                    ...
                    <div class="admin-field">
                        <label for="manual-payment-discount">Zelle / Cash App discount percent</label>
                        <input class="admin-input" id="manual-payment-discount" type="number" min="0" max="100" step="1" name="manual_payment_discount_percent" value="<?= admin_escape((string) creditsoft_site_manual_payment_discount_percent()) ?>">
                        <div class="helper-copy">Checkout and payment matching use this value with the current plan prices.</div>
                    </div>
                    ...
                    <?php if (($pricing['addons'] ?? []) !== []): ?>
                        <div class="section-head" style="margin-top:18px;">
                            <div>
                                ...
                                <h2 style="font-size:22px;">Server node pricing</h2>
                                <p>Checkout invoices pull these add-on prices from the same database price book.</p>
                            </div>
                        </div>
                        <div class="plan-editor-grid">
                            <?php foreach (($pricing['addons'] ?? []) as $addonKey => $addon): ?>
                                <article class="plan-card">
                                    <div class="section-head">
                                        <div>
                                            <span class="eyebrow"><?= admin_escape(str_replace('_', ' ', $addonKey)) ?></span>
                                            <h2 style="font-size:22px;"><?= admin_escape((string) ($addon['name'] ?? 'Add-on')) ?></h2>
                                        </div>
                                    </div>
                                    ...
                                    <div class="admin-form-grid is-two">
                                        <div class="admin-field">
                                            ...
                                            <input class="admin-input" type="text" name="addons[<?= admin_escape($addonKey) ?>][name]" value="<?= admin_escape((string) ($addon['name'] ?? '')) ?>">
                                        </div>
                                        <div class="admin-field">
                                            <label>Short name</label>
                                            <input class="admin-input" type="text" name="addons[<?= admin_escape($addonKey) ?>][short_name]" value="<?= admin_escape((string) ($addon['short_name'] ?? '')) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="price-grid">
                                        <div class="admin-field">
                                            <label>Monthly price</label>
                                            <input class="admin-input" type="number" step="0.01" name="addons[<?= admin_escape($addonKey) ?>][monthly]" value="<?= admin_escape((string) ($addon['monthly'] ?? '0')) ?>">
                                        </div>
                                        <div class="admin-field">
                                            <label>Monthly list price</label>
                                            <input class="admin-input" type="number" step="0.01" name="addons[<?= admin_escape($addonKey) ?>][list_monthly]" value="<?= admin_escape((string) ($addon['list_monthly'] ?? '0')) ?>">
                                        </div>
                                        <div class="admin-field">
                                            <label>Yearly price</label>
                                            <input class="admin-input" type="number" step="0.01" name="addons[<?= admin_escape($addonKey) ?>][yearly]" value="<?= admin_escape((string) ($addon['yearly'] ?? '0')) ?>">
                                        </div>
                                        <div class="admin-field">
                                            <label>Yearly list price</label>
                                            <input class="admin-input" type="number" step="0.01" name="addons[<?= admin_escape($addonKey) ?>][list_yearly]" value="<?= admin_escape((string) ($addon['list_yearly'] ?? '0')) ?>">
                                        </div>
                                    </div>

                                    <div class="admin-field">
                                        <label>Description</label>
                                        <textarea class="admin-textarea" name="addons[<?= admin_escape($addonKey) ?>][description]"><?= admin_escape((string) ($addon['description'] ?? '')) ?></textarea>
                                    </div>
                                    <div class="admin-field">
                                        <label>Features</label>
                                        <textarea class="admin-textarea" name="addons[<?= admin_escape($addonKey) ?>][features]"><?= admin_escape(implode("\n", (array) ($addon['features'] ?? []))) ?></textarea>
                                    </div>
                                </article>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="helper-copy">Pricing source: <?= admin_escape(creditsoft_site_pricing_source_label()) ?></div>
                    <div class="autosave-note"><span>Tab out of a field and CreditSoft saves the database price book for you.</span><span class="autosave-status" aria-live="polite"></span></div>
                </form>
            </section>
            <?php endif; ?>
            ...
            <section class="card section-card<?= admin_panel_visible('licenses', $panel) ?>">
                ...
                <?php else: ?>
                    <div class="stats-inline">
                        ...
                        <div class="mini-card"><span class="eyebrow">Monthly terms</span><div class="stat-value" style="font-size:34px;"><?= (int) ($licenseStats['monthly_terms'] ?? 0) ?></div></div>
                    </div>
                    ...
                    <?php else: ?>
                        <div class="table-wrap">
                            <table>
                                <thead><tr><th>Customer</th><th>Plan</th><th>Billing</th><th>Status</th><th>Amount</th><th>Expires</th><th>Last payment</th><th>License type</th><th>Archive</th></tr></thead>
                                <tbody>
                                <?php foreach ($licenses as $license): ?>
                                    <?php
                                        $licenseKey = trim((string) ($license['license_key'] ?? ''));
                                        $licenseCustomerName = trim((string) (($license['customer_name'] ?: $license['customer_email']) ?? 'Unknown customer'));
                                        $licenseClientCount = cs_site_admin_client_count_label($license['client_count'] ?? '', true);
                                        $licenseWorkflow = cs_site_admin_workflow_label($license['current_workflow'] ?? '', true);
                                    ?>
                                    <tr>
                                        <td>
                                            <button
                                                type="button"
                                                class="license-copy-name"
                                                data-license-copy="<?= admin_escape($licenseKey) ?>"
                                                title="Copy license key"
                                            >
                                                <strong><?= admin_escape($licenseCustomerName) ?></strong>
                                                <small><?= admin_escape((string) ($license['customer_email'] ?? '')) ?></small>
                                                <small><?= admin_escape($licenseClientCount) ?> · <?= admin_escape($licenseWorkflow) ?></small>
                                                <small class="license-copy-status" aria-live="polite">Click customer name to copy license key</small>
                                            </button>
                                        </td>
                                        <td><strong><?= admin_escape(cs_site_admin_badge_label((string) ($license['plan'] ?? ''))) ?></strong></td>
                                        <td><strong><?= admin_escape(cs_site_admin_badge_label((string) ($license['billing_cycle'] ?? 'one time'))) ?></strong></td>
                                        ...
                                        <td><strong><?= admin_format_datetime($license['last_payment_at'] ?? null) ?></strong><small><?= ! empty($license['last_payment_at']) ? 'Payment recorded' : '' ?></small></td>
                                        <td><strong><?= admin_escape((string) ($license['domain'] ?: 'Intranet license')) ?></strong><small>Key hidden in admin list</small></td>
                                        <td>
                                            <form method="post" class="archive-inline-form" onsubmit="return confirm('Archive this customer, license, payment history, and related setup records from active admin views?');">
                                                <input type="hidden" name="csrf" value="<?= admin_escape(cs_site_admin_csrf_token()) ?>">
                                                <input type="hidden" name="action" value="archive_customer">
                                                <input type="hidden" name="panel" value="licenses">
                                                <input type="hidden" name="customer_email" value="<?= admin_escape((string) ($license['customer_email'] ?? '')) ?>">
                                                <select class="admin-select" name="archive_reason" aria-label="Archive reason">
                                                    <option value="testing">Testing</option>
                                                    <option value="duplicate">Duplicate</option>
                                                    <option value="mistaken payment">Mistaken payment</option>
                                                    <option value="non-customer">Non-customer</option>
                                                </select>
                                                <button type="submit" class="hero-btn">Archive</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
            ...
            <section class="card section-card payment-section<?= admin_panel_visible('payments', $panel) ?>">
                ...
                <div class="payment-command">
                    <article class="payment-command-panel is-mailbox">
                        <div class="payment-command-title is-compact">
                            ...
                            <div class="payment-title-stack">
                                <div class="payment-title-line">
                                    <strong><?= admin_escape((string) ($zellePayments['mailbox'] ?? 'Payment mailbox')) ?></strong>
                                </div>
                            </div>
                        </div>
                        <div class="payment-brand-strip">
                            <span class="payment-brand-logo" title="Zelle auto-match" aria-label="Zelle auto-match"><?= admin_payment_brand_logo('zelle') ?></span>
                            <span class="payment-brand-logo" title="Cash App trusted forwarder" aria-label="Cash App trusted forwarder"><?= admin_payment_brand_logo('cashapp') ?></span>
                        </div>
                        <div class="payment-health-list">
                            <div class="payment-health-row">
                                ...
                                <span class="payment-health-copy">
                                    <strong><?= ! empty($zellePayments['configured']) ? 'Mailbox auth configured' : 'Mailbox auth missing' ?></strong>
                                    <span><?= ! empty($zellePayments['imap_enabled']) ? 'Private config or env. Change it on the server.' : 'Add the IMAP password in private config.' ?></span>
                                </span>
                            </div>
                            <div class="payment-health-row">
                                <span class="payment-mini-icon"><?= admin_payment_icon('database') ?></span>
                                <span class="payment-health-copy">
                                    <strong><?= ! empty($zellePayments['available']) ? 'Payment table connected' : 'Payment data is limited' ?></strong>
                                    <span><?= ! empty($zellePayments['error']) ? admin_escape((string) $zellePayments['error']) : (int) ($zellePayments['stats']['total'] ?? 0) . ' payment messages stored.' ?></span>
                                </span>
                                <?php if (empty($zellePayments['available'])): ?><span class="payment-state-dot is-alert" aria-hidden="true"></span><?php endif; ?>
                            </div>
                            <div class="payment-mailbox-meta">
                                Source: <?= admin_escape(implode(' · ', array_filter([(string) ($zellePayments['mailbox'] ?? ''), 'Outbound: ' . (string) ($zellePayments['from_email'] ?? '')], static fn (string $value): bool => trim($value) !== '' && trim($value) !== 'Outbound:'))) ?>
                            </div>
                        </div>
                    </article>
                    <article class="payment-command-panel is-analytics">
                        <div class="payment-command-title">
                            ...
                            <div class="payment-title-stack">
                                ...
                                <span class="payment-muted">What needs attention right now.</span>
                            </div>
                        </div>
                        <div class="payment-metric-grid">
                            <div class="payment-metric is-good">
                                ...
                                <span class="payment-muted">Issued</span>
                            </div>
                            <div class="payment-metric<?= (($zellePayments['stats']['needs_review'] ?? 0) > 0) ? ' is-alert' : '' ?>">
                                ...
                                <span class="payment-muted">Needs action</span>
                            </div>
                            <div class="payment-metric<?= (($zellePayments['stats']['balance_due'] ?? 0) > 0) ? ' is-alert' : '' ?>">
                                ...
                                <span class="payment-muted">Balance due</span>
                            </div>
                            <div class="payment-metric<?= (($zellePayments['stats']['overpaid'] ?? 0) > 0) ? ' is-credit' : '' ?>">
                                <div class="payment-metric-top"><span class="payment-metric-label">Credits</span><span class="payment-mini-icon"><?= admin_payment_icon('credit') ?></span></div>
                                <span class="payment-metric-value"><?= (int) ($zellePayments['stats']['overpaid'] ?? 0) ?></span>
                                <span class="payment-muted">Overpaid</span>
                            </div>
                            <div class="payment-metric<?= (($zellePayments['stats']['open_tickets'] ?? 0) > 0) ? ' is-alert' : '' ?>">
                                ...
                                <span class="payment-muted">Open</span>
                            </div>
                            <div class="payment-metric is-good">
                                <div class="payment-metric-top"><span class="payment-metric-label">License emails</span><span class="payment-mini-icon"><?= admin_payment_icon('mailbox') ?></span></div>
                                <span class="payment-metric-value"><?= (int) ($zellePayments['stats']['email_sent'] ?? 0) ?></span>
                                <span class="payment-muted">Sent</span>
                            </div>
                            <div class="payment-metric">
                                ...
                                <span class="payment-muted">Stored</span>
                            </div>
                        </div>
                        <?php
                            $salesSummary = is_array($zellePayments['sales'] ?? null) ? $zellePayments['sales'] : [];
                            $salesProviders = is_array($salesSummary['provider_rows'] ?? null) ? $salesSummary['provider_rows'] : [];
                            $salesChartPayload = admin_payment_sales_chart_payload($salesSummary);
                            $topProviderByCount = $salesProviders;
                            usort($topProviderByCount, static fn (array $left, array $right): int => ((int) ($right['count'] ?? 0)) <=> ((int) ($left['count'] ?? 0)));
                            $topProviderByRevenue = $salesProviders;
                            usort($topProviderByRevenue, static fn (array $left, array $right): int => ((float) ($right['amount'] ?? 0)) <=> ((float) ($left['amount'] ?? 0)));
                            $topUsedProvider = $topProviderByCount[0] ?? ['label' => 'None yet', 'count' => 0];
                            $topRevenueProvider = $topProviderByRevenue[0] ?? ['label' => 'None yet', 'amount' => 0];
                            $salesGrossTotal = (float) ($salesSummary['total'] ?? 0);
                            $salesNetTotal = (float) ($salesSummary['net_total'] ?? $salesGrossTotal);
                            $cashAppFeeTotal = (float) ($salesSummary['fee_total'] ?? 0);
                            $cashAppFeePercent = (float) ($salesSummary['cash_app_fee_percent'] ?? 0);
                            $salesChartStartLabel = trim((string) ($salesChartPayload['daily']['labels'][0] ?? ''));
                            $salesChartEndLabel = (new DateTimeImmutable('now', new DateTimeZone('America/Los_Angeles')))->format('M j, Y');
                            $salesChartRangeLabel = $salesChartStartLabel !== ''
                                ? $salesChartStartLabel . ' - ' . $salesChartEndLabel
                                : 'Through ' . $salesChartEndLabel;
                        ?>
                        <div class="payment-sales-card">
                            <div class="payment-sales-head">
                                <div>
                                    <strong>Payment analytics</strong>
                                    <span><?= admin_escape($salesChartRangeLabel) ?></span>
                                </div>
                                <div class="payment-sales-total"><?= admin_escape(cs_site_zelle_money($salesGrossTotal)) ?></div>
                            </div>
                            <script type="application/json" id="paymentAnalyticsData"><?= admin_json($salesChartPayload) ?></script>
                            <div class="payment-kpi-row">
                                <div class="payment-kpi">
                                    <span>Gross received</span>
                                    <strong><?= admin_escape(cs_site_zelle_money($salesGrossTotal)) ?></strong>
                                    <small><?= (int) ($salesSummary['count'] ?? 0) ?> paid payments</small>
                                </div>
                                <div class="payment-kpi">
                                    <span>Net after fees</span>
                                    <strong><?= admin_escape(cs_site_zelle_money($salesNetTotal)) ?></strong>
                                    <small>Cash App estimated; Zelle fee $0</small>
                                </div>
                                <div class="payment-kpi">
                                    <span>Cash App fees</span>
                                    <strong><?= admin_escape(cs_site_zelle_money($cashAppFeeTotal)) ?></strong>
                                    <small><?= rtrim(rtrim(number_format($cashAppFeePercent, 2), '0'), '.') ?>% business fee estimate</small>
                                </div>
                                <div class="payment-kpi">
                                    <span>Used most</span>
                                    <strong><?= admin_escape((string) ($topUsedProvider['label'] ?? 'None yet')) ?></strong>
                                    <small><?= (int) ($topUsedProvider['count'] ?? 0) ?> payments · most money: <?= admin_escape((string) ($topRevenueProvider['label'] ?? 'None yet')) ?></small>
                                </div>
                            </div>
                            <div class="payment-chart-empty<?= $salesGrossTotal <= 0 ? ' is-visible' : '' ?>">No processed payment totals are available for this window yet.</div>
                            <div class="payment-analytics-grid">
                                <article class="payment-chart-card">
                                    <div class="payment-chart-head">
                                        <div class="payment-chart-title">
                                            <strong>Gross revenue</strong>
                                            <span>Zelle and Cash App by day</span>
                                        </div>
                                        <div class="payment-chart-stat">
                                            <strong><?= admin_escape(cs_site_zelle_money($salesGrossTotal)) ?></strong>
                                            <span>gross</span>
                                        </div>
                                    </div>
                                    <div class="payment-chart-wrap"><canvas id="paymentRevenueChart" aria-label="Daily gross revenue by payment method"></canvas></div>
                                </article>
                                <article class="payment-chart-card">
                                    <div class="payment-chart-head">
                                        <div class="payment-chart-title">
                                            <strong>Payment mix</strong>
                                            <span>Usage count by lane</span>
                                        </div>
                                    </div>
                                    <div class="payment-chart-wrap is-small"><canvas id="paymentMixChart" aria-label="Payment method usage mix"></canvas></div>
                                </article>
                            </div>
                            <div class="payment-analytics-grid is-secondary">
                                <article class="payment-chart-card">
                                    <div class="payment-chart-head">
                                        <div class="payment-chart-title">
                                            <strong>Top earning lines</strong>
                                            <span>Plan and billing combinations</span>
                                        </div>
                                    </div>
                                    <div class="payment-chart-wrap is-small"><canvas id="paymentPlanChart" aria-label="Top payment revenue by plan"></canvas></div>
                                </article>
                                <article class="payment-chart-card">
                                    <div class="payment-chart-head">
                                        <div class="payment-chart-title">
                                            <strong>Fee drag</strong>
                                            <span>Cash App business cost versus net</span>
                                        </div>
                                        <div class="payment-chart-stat">
                                            <strong><?= admin_escape(cs_site_zelle_money($cashAppFeeTotal)) ?></strong>
                                            <span>fees</span>
                                    ...
                                    <div class="payment-chart-wrap is-small"><canvas id="paymentFeeChart" aria-label="Gross revenue, Cash App fees, and net revenue"></canvas></div>
                                </article>
                            </div>
                        </div>
                    </article>
                </div>
                <div class="payment-table-note"><span class="payment-mini-icon"><?= admin_payment_icon('review') ?></span><span>Review queue: manually assign old payments or retry new checkout matches.</span></div>
                <?php $paymentActionItems = array_values(array_filter((array) ($zellePayments['messages'] ?? []), static fn (array $message): bool => in_array((string) ($message['status'] ?? ''), ['needs_review', 'balance_due'], true))); ?>
                <?php if ($paymentActionItems !== []): ?>
                    <div class="payment-message-list">
                        <?php foreach (array_slice($paymentActionItems, 0, 3) as $message): ?>
                            <article class="payment-message-card is-<?= admin_escape(preg_replace('/[^a-z0-9_-]+/i', '', (string) ($message['status'] ?? 'needs_review')) ?: 'needs_review') ?>">
                                <div class="payment-message-primary">
                                    <span class="payment-mini-icon"><?= admin_payment_icon('review') ?></span>
                                    <div class="payment-cell">
                                        <span class="payment-cell-label">Needs review</span>
                                        <strong><?= admin_escape(admin_payment_sender_name($message)) ?></strong>
                                ...
                                <div class="payment-cell">
                                    ...
                                    <span><?= admin_escape(cs_site_admin_badge_label((string) ($message['payment_provider'] ?? 'payment'))) ?></span>
                                </div>
                                <div class="payment-cell">
                                    <span class="payment-cell-label">Status</span>
                                    <strong><?= admin_escape(cs_site_admin_badge_label((string) ($message['payment_status'] ?? 'review'))) ?></strong>
                                ...
                                <div class="payment-actions">
                                    ...
                                    <form method="post" style="margin:0;">
                                        ...
                                        <input type="hidden" name="amount" value="<?= admin_escape((string) ($message['amount'] ?? '')) ?>">
                                        <button type="submit" class="hero-btn is-primary">Assign and issue</button>
                                    </form>
                                </div>
                            </article>
                        <?php endforeach; ?>
            ...
            <section class="card section-card payment-section<?= admin_panel_visible('payments', $panel) ?>">
                <div class="section-head">
                    <div class="section-head-copy">
                        ...
                        <p>Customer-submitted “paid but no license” requests.</p>
                    </div>
                    <div class="form-actions">
            ...
            <section class="card section-card payment-section<?= admin_panel_visible('payments', $panel) ?>">
                <div class="section-head">
                    <div class="section-head-copy">
                        ...
                        <p>Review unmatched payments first.</p>
                    </div>
                </div>
                ...
                <?php if ($zelleReviewMessages === []): ?>
                    <div class="empty">No payment messages need action.</div>
                <?php else: ?>
                    <div class="payment-message-list">
                            <?php foreach (array_slice($zelleReviewMessages, 0, 8) as $message): ?>
                                <?php
                                    ...
                                    $amountContext = admin_payment_amount_context($message);
                                    $expectedAmount = $amountContext['expected_amount'];
                                    $planAmount = $amountContext['plan_amount'];
                                    $discountAmount = $amountContext['discount_amount'];
                                    $discountPercent = $amountContext['discount_percent'];
                                    $messageMeta = json_decode((string) ($message['metadata_json'] ?? ''), true);
                                    $messageMeta = is_array($messageMeta) ? $messageMeta : [];
                                    ...
                                    $creditForward = is_numeric($expectedPaymentMeta['credit_forward'] ?? null)
                                        ? (float) $expectedPaymentMeta['credit_forward']
                                        : (
                                            isset($message['amount'], $message['expected_amount'])
                                            && is_numeric($message['amount'])
                                            && is_numeric($message['expected_amount'])
                                            && (float) $message['amount'] > ((float) $message['expected_amount'] + 0.02)
                                                ? round((float) $message['amount'] - (float) $message['expected_amount'], 2)
                                                : 0.0
                                        );
                                ...
                                <article class="payment-message-card is-<?= admin_escape($messageStatusClass) ?>">
                                    ...
                                    <div class="payment-cell">
                                        <span class="payment-cell-label">Amount</span>
                                        <strong><?= admin_escape(cs_site_zelle_money(isset($message['amount']) ? (float) $message['amount'] : null)) ?></strong>
                                        <?php if ($planAmount !== null): ?><span>Plan price <?= admin_escape(cs_site_zelle_money($planAmount)) ?></span><?php endif; ?>
                                        <?php if ($expectedAmount !== null && ($planAmount === null || abs($planAmount - $expectedAmount) > 0.02)): ?><span>Manual pay total <?= admin_escape(cs_site_zelle_money($expectedAmount)) ?></span><?php endif; ?>
                                        <?php if ($discountAmount !== null && $discountAmount > 0): ?><span><?= $discountPercent !== null && $discountPercent > 0 ? (int) $discountPercent . '% discount' : 'Discount' ?> <?= admin_escape(cs_site_zelle_money($discountAmount)) ?></span><?php endif; ?>
                                        <?php if ($creditForward > 0): ?><span>Credit next month <?= admin_escape(cs_site_zelle_money($creditForward)) ?></span><?php endif; ?>
            ...
            <section class="card section-card payment-section<?= admin_panel_visible('payments', $panel) ?>">
                <div class="section-head">
                    <div class="section-head-copy">
                        ...
                        <h2>Checkout notices</h2>
                        <p>Checkout and renewal intents waiting for payment mail.</p>
                    </div>
                </div>
            ...
            <section class="card section-card<?= admin_panel_visible('customers', $panel) ?>">
                ...
                <?php else: ?>
                    ...
                    <div class="table-wrap">
                        <table>
	                            <thead><tr><th>Customer</th><th>Company</th><th>Phone</th><th>Lead status</th><th>Licenses</th><th>Plans</th><th>Latest expiration</th><th>Archive</th></tr></thead>
                            <tbody>
                                    ...
                                    <td><strong><?= admin_escape(cs_site_admin_badge_label((string) ($customer['lead_status'] ?? 'not tracked'))) ?></strong></td>
                                        ...
                                        <td>
                                            <form method="post" class="archive-inline-form" onsubmit="return confirm('Archive this customer, license, payment history, and related setup records from active admin views?');">
                                                ...
                                                <input type="hidden" name="action" value="archive_customer">
                                                <input type="hidden" name="panel" value="customers">
                                                <input type="hidden" name="customer_email" value="<?= admin_escape((string) ($customer['customer_email'] ?? '')) ?>">
                                                <select class="admin-select" name="archive_reason" aria-label="Archive reason">
                                                    <option value="testing">Testing</option>
                                                    <option value="duplicate">Duplicate</option>
                                                    <option value="mistaken payment">Mistaken payment</option>
...
<?php if ($panel === 'payments'): ?>
<script defer src="<?= admin_escape(cs_site_public_url('/assets/vendor/chart.umd.min.js')) ?>"></script>
<script>
(() => {
    const whenIdle = window.requestIdleCallback || ((callback) => window.setTimeout(callback, 1));

    const initPaymentCharts = () => {
        const dataNode = document.getElementById('paymentAnalyticsData');
        ...
        const moneyTick = (value) => money.format(Number(value || 0));
        ...
        const chartBase = {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (context) => `${context.dataset.label}: ${moneyTick(context.parsed.y ?? context.parsed.x ?? context.raw)}`,
                    },
                },
            },
            scales: {
                ...
                y: {
                    ...
                    ticks: { color: colors.muted, callback: moneyTick },
                },
            },
        };
        ...
        const revenueCanvas = document.getElementById('paymentRevenueChart');
        if (revenueCanvas) {
        ...
        const mixCanvas = document.getElementById('paymentMixChart');
        if (mixCanvas) {
            new Chart(mixCanvas, {
                type: 'doughnut',
                data: {
                    ...
                    datasets: [{
                        label: 'Payments',
                        data: providers.counts || [],
                        backgroundColor: [colors.zelle, colors.cashApp, colors.ink],
                        borderWidth: 0,
                        hoverOffset: 8,
                    }],
                },
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: (context) => `${context.label}: ${Number(context.raw || 0)} payments`,
                            },
                        },
                    },
                    scales: {},
        ...
        const planCanvas = document.getElementById('paymentPlanChart');
        if (planCanvas) {
            new Chart(planCanvas, {
                ...
                options: {
                    ...
                    indexAxis: 'y',
                    scales: {
                        x: { beginAtZero: true, grid: { color: colors.grid }, ticks: { color: colors.muted, callback: moneyTick } },
        ...
        const feeCanvas = document.getElementById('paymentFeeChart');
        if (feeCanvas) {
            new Chart(feeCanvas, {
                type: 'bar',
                data: {
                    labels: ['Gross', 'Cash App fees', 'Net'],
                    datasets: [{
                        label: `Fee model (${Number(analytics.feePercent || 0)}%)`,
                        data: [totals.gross || 0, totals.fees || 0, totals.net || 0],
                        backgroundColor: [colors.ink, colors.fees, colors.net],
                        borderRadius: 9,
                    }],
                },
                options: {
    ...
    window.addEventListener('load', () => whenIdle(initPaymentCharts), { once: true });
})();
</script>
<?php endif; ?>
<script>
(() => {
    const copyButtons = document.querySelectorAll('[data-license-copy]');
    ...
    copyButtons.forEach((button) => {
        const originalStatus = button.querySelector('.license-copy-status')?.textContent || 'Click customer name to copy license key';

        button.addEventListener('click', async () => {
            const key = button.getAttribute('data-license-copy') || '';
            const status = button.querySelector('.license-copy-status');

            if (!key) {
                if (status) {
                    status.textContent = 'No license key saved for this customer';
                }
            ...
            try {
                ...
                if (status) {
                    status.textContent = 'License key copied';
                    window.setTimeout(() => {
                ...
                if (status) {
                    status.textContent = 'Copy failed; open details to copy manually';
                }
            }
        });
    });
