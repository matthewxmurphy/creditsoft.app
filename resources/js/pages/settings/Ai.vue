...
const form = useForm({
    default_provider: props.defaultProvider || 'openrouter_creditsoft',
    opencode_api_key: '',
    openrouter_api_key: '',
...
const configuredProviders = computed<Record<string, boolean>>(() => {
    return Object.fromEntries(
        (props.catalog.providers ?? []).map((provider) => [provider.name, provider.configured]),
    );
});

const maskedKeys = computed<Record<string, string | null>>(() => {
    return Object.fromEntries(
        (props.catalog.providers ?? []).map((provider) => [provider.name, provider.masked_key ?? null]),
    );
});
...
const submit = () => {
    form.put('/settings/ai', {
        ...
        onSuccess: () => {
            ...
            router.reload({
                only: ['catalog', 'defaultProvider', 'creditsoft'],
            });
        },
    });
};
...
<template>
    ...
    <div class="space-y-6">
        <section class="space-y-2">
            <h1 class="text-xl font-semibold text-stone-950">AI provider setup</h1>
            <p class="text-sm text-stone-600">
                Paste any provider keys you want to save, then choose which one should be the default for drafting and summaries.
            </p>
        </section>

        <form class="overflow-hidden rounded-[28px] border border-stone-300/70 bg-white/90" @submit.prevent="submit">
            ...
            <div class="flex flex-col gap-4 border-t border-stone-200/80 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
                <p class="text-sm text-stone-600">
